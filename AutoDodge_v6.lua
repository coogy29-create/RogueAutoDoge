...
local function analyzeDenseFront(root, humanoid, threats)
...
Footer.Text = "1.5s MPC / FAN Escape / Async Planner"
...
local function decisionStep()
	...
	local ok, err = pcall(function()
		...
		local denseFront = analyzeDenseFront(root, humanoid, threats)
		if denseFront then
		...
		-- Double-buffer behavior: CurrentMove changes only after a complete plan exists.
...
-- RenderStep only applies the last completed move.
