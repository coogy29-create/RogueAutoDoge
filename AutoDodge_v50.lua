...
local V50 = {
	C = {
		...
		SHIELD_PRESS_TIME = 0.12,
		SHIELD_CONFIRM_TIMEOUT = 0.55,
		SHIELD_RETRY_DELAY = 0.18,
		SHIELD_PENDING_LOCK = 0.70,
		SHIELD_REQUIRED_CHARGE = 2,
		SHIELD_TRIGGER_IMPACT = 0.90,
		SHIELD_PANIC_IMPACT = 0.38,
		SHIELD_FRAGILE_CLEARANCE = 0.16,
		SHIELD_FRAGILE_ESCAPE_OPTIONS = 1,
		SHIELD_CLEARANCE_GAIN = 0.22,
		SHIELD_ESCAPE_GAIN = 1,
		SHIELD_MIN_REMOVED = 1,
		SHIELD_PLAN_HOLD = 0.12,

		UMBRELLA_COUNT = 2,
		UMBRELLA_SPREAD_ANGLE = math.rad(8.5),
		UMBRELLA_SPAWN_BACK = 5.0,
		UMBRELLA_SPEED = 46.5,
		UMBRELLA_LIFETIME = 2.03,
		UMBRELLA_MAX_DISTANCE = 90.0,
		UMBRELLA_ACTIVATION_DELAY = 0.10,
		UMBRELLA_DELETE_RADIUS = 4.70,
		UMBRELLA_START_RADIUS = 0.25,
		UMBRELLA_GROW_TIME = 0.24,
		UMBRELLA_MIN_REMOVED = 2,
		UMBRELLA_MIN_CLEARANCE_GAIN = 0.18,
		UMBRELLA_ESCAPE_GAIN = 1,
		UMBRELLA_FRAGILE_CLEARANCE = 0.30,
		UMBRELLA_CONFIRM_TIMEOUT = 0.50,
		UMBRELLA_FAILED_BACKOFF_BASE = 3.0,
		UMBRELLA_FAILED_BACKOFF_MAX = 18.0,
		UMBRELLA_CONFIRMED_GUARD = 2.15,
		UMBRELLA_SHIELD_PANIC_TIME = 0.38,
		...
		RETURN_HISTORY_MAX = 11,
		RETURN_HISTORY_INTERVAL = 0.028,
		RETURN_MIN_TRAVEL = 2.0,
		RETURN_MIN_RADIUS = 0.65,
		RETURN_MAX_RADIUS = 90.0,
		RETURN_FIT_TOLERANCE = 0.18,
		RETURN_CONFIDENCE_ON = 0.52,
		RETURN_CONFIDENCE_OFF = 0.34,
		RETURN_PREVIEW_TIME = 0.40,
		RETURN_PREVIEW_GAIN = 0.32,
		RETURN_RADIAL_SPEED = 1.2,
		RETURN_MAX_ANGULAR_RATE = math.rad(1080),
		RETURN_RATE_SMOOTH = 0.42,
		RETURN_EXTRA_MARGIN = 0.22,
		RETURN_MAX_PREDICTION_TIME = 1.45,

		RETURN_STANDOFF = 0.95,
		RETURN_STANDOFF_WEIGHT = 165.0,
		RETURN_CORRIDOR_ZONE = 1.85,
		RETURN_SIDE_SHRINK_WEIGHT = 150.0,
		RETURN_CROSS_WEIGHT = 520.0,
		RETURN_CENTERLINE_TANGENT_WEIGHT = 65.0,
		RETURN_SIDE_EPSILON = 0.10,
		RETURN_SIDE_LOOKAHEAD = 1.00,
		RETURN_EMERGENCY_CLEARANCE = 0.55,
		RETURN_GRAZE_BLOCK_ZONE = 1.30,

		MPC_EXEC_NORMAL_AGE = 0.32,
		MPC_EXEC_CORNER_AGE = 0.38,
		MPC_EXEC_GRAZE_AGE = 0.16,
		MPC_EXEC_RESCUE_AGE = 0.14,
		MPC_EXEC_POSITION_ERROR = 1.35,
		MPC_EXEC_WALL_CHECK_TIME = 0.10,

		DENSE_SAFE_COUNT = 18,
		DENSE_CRITICAL_COUNT = 30,
		DENSE_SAFE_IMMEDIATE = 7,
		DENSE_CRITICAL_IMMEDIATE = 12,
		DENSE_RADIUS_BOOST = 0.10,
		DENSE_CRITICAL_RADIUS_BOOST = 0.16,
		DENSE_EXEC_AGE = 0.095,
		DENSE_CRITICAL_EXEC_AGE = 0.060,
		DENSE_FORCE_REPLAN_CLEARANCE = 1.25,
		DENSE_SHIELD_CLEARANCE = 0.34,
		DENSE_SHIELD_IMMEDIATE = 10
	},
	State = {
		CornerActive = false,
		CornerDirection = nil,
		...
		ShieldPending = false,
		ShieldPendingUntil = 0,
		ShieldPendingStarted = 0,
		ShieldRetryCount = 0,
		ShieldSimMs = 0,
		ShieldCrisisReason = "-",

		UmbrellaPending = false,
		UmbrellaPendingUntil = 0,
		UmbrellaNextAttempt = 0,
		UmbrellaFailStreak = 0,
		UmbrellaLastDecision = "IDLE",
		UmbrellaLastRemoved = 0,
		UmbrellaSimMs = 0,
		UmbrellaUseCount = 0,
		...
		ReturnMinClearance = math.huge,
		ReturnStandoffActive = false,
		...
		DenseSafetyActive = false,
		DenseCritical = false,
		DenseRadiusBoost = 0,
		DenseExactCount = 0,
		DenseImmediateCount = 0,
		GrazeSuppressed = false
	}
}
...
local function addProjectile(obj)
	task.defer(function()
		...
		ProjectileData[obj] = {
			...
			InitialDirection =
				initialFlat.Magnitude > 0.01
					and initialFlat.Unit
					or Vector3.zero,
			MaxSpawnDistance = 0,
			MotionHistory = {},
			LastHistoryTime = 0,
...
function V50.directionOpenDistance(position, direction)
	local nearest = V50.C.CORNER_PROBE_DISTANCE
...
function V50.bestCornerExit(position)
	local bestDirection = nil
	local bestScore = -math.huge

	for i = 0, V50.C.CORNER_PROBE_DIRECTIONS - 1 do
		local angle =
			i * math.pi * 2
				/ V50.C.CORNER_PROBE_DIRECTIONS
...
function V50.updateCornerObjective(position)
	local _, minimumOpen, closeRays =
		barrierOpenness(position)

	-- Once we have actually left the corner, release the objective.
	if minimumOpen >= V50.C.CORNER_RELEASE_OPEN
		and closeRays < V50.C.CORNER_CLOSE_RAYS
	then
		V50.State.CornerActive = false
		V50.State.CornerDirection = nil
		return nil
	end

	local trapped =
		minimumOpen < V50.C.CORNER_MIN_OPEN
		and closeRays >= V50.C.CORNER_CLOSE_RAYS

	-- Critical anti-wiggle behavior:
	-- keep the SAME exit direction as long as that channel still exists.
	if V50.State.CornerActive
		and V50.State.CornerDirection
	then
		local clearance =
			V50.directionOpenDistance(
				position,
				V50.State.CornerDirection
			)

		if clearance
			>= V50.C.CORNER_EXIT_MIN_CLEARANCE
		then
			return V50.State.CornerDirection
		end
	...
	local direction =
		V50.bestCornerExit(position)

	if direction then
		V50.State.CornerActive = true
		V50.State.CornerDirection = direction
	end

	return direction
end

function V50.cornerRouteCost(
	oldPosition,
	newPosition,
	direction,
	cornerDirection
...
local function updateMotionClass(data, rawTurn, dt, now)
	if data.MotionClass == "RETURN"
		and (data.ReturnConfidence or 0)
			>= V50.C.RETURN_CONFIDENCE_OFF
	then
		return
	end
...
function V50.pushMotionHistory(data, position, now)
	local history = data.MotionHistory
	if not history then
		history = {}
		data.MotionHistory = history
	end

	local lastTime = data.LastHistoryTime or 0

	if #history > 0
		and now - lastTime
			< V50.C.RETURN_HISTORY_INTERVAL
	then
		history[#history].P = position
		history[#history].T = now
		return
	end
	...
	data.LastHistoryTime = now

	while #history > V50.C.RETURN_HISTORY_MAX do
		table.remove(history, 1)
	end
end

function V50.fitReturnArc(data)
	local history = data.MotionHistory
	...
	local firstIndex = 1
	local middleIndex =
		math.floor((#history + 1) * 0.5)
	local lastIndex = #history

	local a = history[firstIndex]
	local b = history[middleIndex]
	local c = history[lastIndex]

	local x1, z1 = a.P.X, a.P.Z
	local x2, z2 = b.P.X, b.P.Z
	local x3, z3 = c.P.X, c.P.Z
	...
	local q1 = x1 * x1 + z1 * z1
	local q2 = x2 * x2 + z2 * z2
	local q3 = x3 * x3 + z3 * z3
	...
	local radius =
		flat(c.P - center).Magnitude

	if radius < V50.C.RETURN_MIN_RADIUS
		or radius > V50.C.RETURN_MAX_RADIUS
	then
		return nil
	end
	...
	for _, sample in ipairs(history) do
		local sampleRadius =
			flat(sample.P - center).Magnitude

		totalError +=
			math.abs(sampleRadius - radius)
	end
	...
	-- Measure angular rate from the newest separated samples.
	local older =
		history[
			math.max(
				1,
	...
	if oldRadial.Magnitude <= 0.01
		or newRadial.Magnitude <= 0.01
	then
		return nil
	end
...
function V50.predictReturnArcPoint(data, t, origin)
	local center = data.ReturnCenter
	local rate = data.ReturnAngularRate or 0

	if not center
		or math.abs(rate) < 0.001
	then
		return nil
	end

	local basePosition =
		origin or data.LastPosition

	local radial =
		flat(basePosition - center)
	...
	local angle =
		rate
			* math.min(
				t,
				V50.C.RETURN_MAX_PREDICTION_TIME
			)
...
function V50.updateReturnModel(
	data,
	position,
	rawVelocity,
	now,
	...
	if not data.SpawnPosition then
		data.SpawnPosition = position
	end

	local flatVelocity = flat(rawVelocity)
	local speed = flatVelocity.Magnitude

	if (not data.InitialDirection
			or data.InitialDirection.Magnitude <= 0.01)
		and speed >= MIN_PROJECTILE_SPEED
	then
		data.InitialDirection = flatVelocity.Unit
	end

	V50.pushMotionHistory(
		data,
		position,
		now
	)
	...
	local displacement =
		flat(position - data.SpawnPosition)
	...
	data.MaxSpawnDistance =
		math.max(
			data.MaxSpawnDistance or 0,
			spawnDistance
		)

	local fit =
		V50.fitReturnArc(data)

	if not fit then
		data.ReturnFitConfidence +=
			(0 - (data.ReturnFitConfidence or 0))
				* math.clamp(dt * 5, 0, 1)

		data.ReturnConfidence +=
			(0 - (data.ReturnConfidence or 0))
				* math.clamp(dt * 3, 0, 1)

		return
	end

	data.ReturnCenter = fit.Center
	data.ReturnRadius = fit.Radius
	data.ReturnFitConfidence =
		fit.FitConfidence

	if math.abs(data.ReturnAngularRate or 0)
		<= 0.001
	then
		data.ReturnAngularRate =
			fit.AngularRate
	else
		data.ReturnAngularRate +=
			(
				fit.AngularRate
					- data.ReturnAngularRate
			) * V50.C.RETURN_RATE_SMOOTH
	end

	local movingBackScore = 0

	if spawnDistance > 0.10
		and speed > 0.10
	then
		local outward =
			displacement.Unit
		...
		local radialVelocity =
			flatVelocity:Dot(outward)

		movingBackScore =
			math.clamp(
				(
					-radialVelocity
						- V50.C.RETURN_RADIAL_SPEED
				) / math.max(speed * 0.45, 1),
				0,
				1
			)
	end
	...
	local retreat =
		(data.MaxSpawnDistance or spawnDistance)
			- spawnDistance
	...
	local headingScore = 0

	if speed > 0.10
		and data.InitialDirection
		and data.InitialDirection.Magnitude > 0.01
	then
		local dot =
			flatVelocity.Unit:Dot(
				data.InitialDirection.Unit
			)
	...
	-- Predict the fitted arc a little ahead. This lets RETURN classification
	-- happen BEFORE the projectile has fully started moving inward.
	local futureInwardScore = 0

	if spawnDistance >= V50.C.RETURN_MIN_TRAVEL then
		local future =
			V50.predictReturnArcPoint(
				data,
				V50.C.RETURN_PREVIEW_TIME,
				position
			)

		if future then
			local futureDistance =
				flat(
					future - data.SpawnPosition
				).Magnitude

			local gain =
				spawnDistance - futureDistance

			futureInwardScore =
				math.clamp(
					(
						gain
							- V50.C.RETURN_PREVIEW_GAIN
					) / 1.20,
					0,
					1
				)
	...
	local rawConfidence =
		travelGate
		* (
			0.34 * fit.FitConfidence
			+ 0.25 * futureInwardScore
			+ 0.21 * movingBackScore
			+ 0.12 * retreatScore
			+ 0.08 * headingScore
		)

	-- If the projectile already has a strong repeated-wave signature,
	-- keep WAVE; RETURN is aimed at one-shot U-turn / boomerang arcs.
	local strongWave =
		(data.WaveConfidence or 0) >= 0.68
		and data.TurnCrossings
		and #data.TurnCrossings >= 2
	...
	data.ReturnConfidence +=
		(
			rawConfidence
				- (data.ReturnConfidence or 0)
		) * math.clamp(dt * 8, 0, 1)

	local oldClass =
		data.MotionClass or "STRAIGHT"

	if oldClass == "RETURN" then
		if data.ReturnConfidence
			>= V50.C.RETURN_CONFIDENCE_OFF
		then
			data.MotionClass = "RETURN"
		else
			data.MotionClass =
				math.abs(data.TurnRate or 0)
					>= CURVE_TURN_THRESHOLD
	...
	elseif data.ReturnConfidence
		>= V50.C.RETURN_CONFIDENCE_ON
	then
		data.MotionClass = "RETURN"
	end
end
...
RunService.Heartbeat:Connect(function(dt)
	...
	for obj, data in pairs(ProjectileData) do
		...
		if not part or not part.Parent then
			...
			data.SpawnPosition = data.LastPosition
			data.InitialDirection = Vector3.zero
			data.MaxSpawnDistance = 0
			data.MotionHistory = {}
			data.LastHistoryTime = 0
			data.ReturnCenter = nil
			data.ReturnRadius = 0
			data.ReturnAngularRate = 0
			data.ReturnFitConfidence = 0
			data.ReturnConfidence = 0
...
local function simulateProjectilePositions(data, networkLead)
	...
	local useReturn =
		data.MotionClass == "RETURN"
			and data.ReturnCenter ~= nil
			and math.abs(
				data.ReturnAngularRate or 0
			) > 0.001

	local useWave =
		not useReturn
			and data.MotionClass == "WAVE"
			and (data.WaveHalfPeriod or 0) > 0
...
local function buildThreatSnapshot(root, humanoid)
	...
	for _, data in pairs(ProjectileData) do
		...
		then
			if data.MotionClass == "RETURN" then
				returnCount += 1
			elseif data.MotionClass == "WAVE" then
				waveCount += 1
			...
			local threat = {
				...
				ReturnPreferredSide = 0,
				ReturnSideDistance = math.huge
			}
			...
			for stepIndex = 1, TOTAL_PREDICTION_STEPS do
				...
				if p0 and p1 then
					...
					if data.MotionClass == "RETURN"
						and t <= V50.C.RETURN_SIDE_LOOKAHEAD
						and stationaryDistance
							< threat.ReturnSideDistance
					then
						local tangent =
							flat(p1 - p0)
						...
						if tangent.Magnitude > 0.01 then
							...
							if math.abs(side)
								> V50.C.RETURN_SIDE_EPSILON
							then
								threat.ReturnPreferredSide =
									side > 0 and 1 or -1
	...
	V50.State.TrajectoryStraight = straightCount
	V50.State.TrajectoryCurve = curveCount
	V50.State.TrajectoryWave = waveCount
	V50.State.TrajectoryReturn = returnCount

	return threats
end

function V50.getShieldCharge()
	...
	if not ok or not rawText then
		return 0
	end

	-- Current charge can exceed 2. The displayed denominator is the
	-- shield requirement/cost, not a maximum capacity.
	local current =
		string.match(
			rawText,
			"(%d+)"
...
function V50.getShieldCooldownRemaining()
	return math.max(
		0,
		V50.C.SHIELD_COOLDOWN
			- (os.clock() - V50.State.ShieldLastUse)
	)
end

function V50.findShieldPart()
	local core = workspace:FindFirstChild("Core__Game")
	if not core then
		return nil
	end
...
function V50.getActiveShieldModel()
	local now = os.clock()
	local shield = V50.findShieldPart()

	if shield then
		if V50.State.ShieldPart ~= shield then
			V50.State.ShieldPart = shield
			V50.State.ShieldCenter = shield.Position
			V50.State.ShieldUntil =
				now + V50.C.SHIELD_DURATION

			-- Also catches a manual shield use while the bot is running.
			if now - V50.State.ShieldLastUse > 1.0 then
				V50.State.ShieldLastUse = now
			end

			if V50.State.ShieldPending then
				V50.State.ShieldPending = false
				V50.State.ShieldPendingUntil = 0
				V50.State.ShieldLastDecision = "CONFIRMED"
			end
		else
			V50.State.ShieldCenter = shield.Position
		end
		...
		local remaining =
			math.max(
				0,
				V50.State.ShieldUntil - now
			)

		if remaining > 0 then
			return {
				Center = V50.State.ShieldCenter,
				StartDelay = 0,
				Duration = remaining
			}
		end
	...
	if V50.State.ShieldPart ~= nil then
		V50.State.ShieldPart = nil
		V50.State.ShieldCenter = nil
		V50.State.ShieldUntil = 0
	end

	return nil
end

function V50.canUseShield()
	local now = os.clock()

	if V50.State.ShieldPending
		and now < V50.State.ShieldPendingUntil
	then
		V50.State.ShieldLastDecision = "PENDING"
		return false
	end
	...
	if charge < V50.C.SHIELD_REQUIRED_CHARGE then
		V50.State.ShieldLastDecision = "NO CHARGE"
		return false
	end

	if V50.getShieldCooldownRemaining() > 0 then
		V50.State.ShieldLastDecision = "COOLDOWN"
		return false
	end

	if V50.getActiveShieldModel() then
		V50.State.ShieldLastDecision = "ACTIVE"
		return false
	end
	...
	if not event or not event:IsA("RemoteEvent") then
		V50.State.ShieldLastDecision = "NO REMOTE"
		return false
	end
...
function V50.useShield(root)
	if not root or not root.Parent then
		return false
	end

	if not V50.canUseShield() then
		return false
	end
	...
	V50.State.ShieldPending = true
	V50.State.ShieldPendingStarted = requestTime
	V50.State.ShieldPendingUntil =
		requestTime + V50.C.SHIELD_PENDING_LOCK
	V50.State.ShieldRetryCount = 0
	V50.State.ShieldLastDecision = "PRESS"
	...
	task.spawn(function()
		...
		do
			...
			-- Charge has no maximum. Confirm by detecting a decrement from
			-- the pre-cast value (the shield costs 2), not by checking <2.
			if charge < initialCharge then
				confirmed = true
				break
			end
		...
		if not confirmed then
			...
			-- Retry only if the charge did not change at all, meaning the
			-- first pulse was not visibly accepted.
			if charge >= initialCharge
				and event
				and event.Parent
			then
				V50.State.ShieldRetryCount = 1
				V50.State.ShieldLastDecision = "RETRY"

				task.wait(
					V50.C.SHIELD_RETRY_DELAY
				)
		...
		if confirmed then
			...
			-- Cooldown starts ONLY after the game actually accepted the cast.
			V50.State.ShieldLastUse = now
			V50.State.ShieldCenter = castOrigin
			V50.State.ShieldUntil =
				now
					+ V50.C.SHIELD_ACTIVATION_DELAY
					+ V50.C.SHIELD_DURATION

			V50.State.ShieldPlanHoldUntil =
				now + V50.C.SHIELD_PLAN_HOLD

			V50.State.ShieldUseCount += 1
			V50.State.ShieldLastDecision =
				V50.State.ShieldRetryCount > 0
					and "CONFIRMED RETRY"
					or "CONFIRMED"
		else
			-- Most important fix: a failed remote attempt does not create a
			-- fake 30 second cooldown anymore.
			V50.State.ShieldLastDecision = "CAST FAILED"
		end
...
function V50.getEffectsFolder()
	local core = workspace:FindFirstChild("Core__Game")
	return core and core:FindFirstChild("Effects")
end

function V50.countActiveUmbrellas()
	local effects = V50.getEffectsFolder()
	if not effects then
		return 0
	end
...
function V50.canAttemptUmbrella()
	local now = os.clock()

	if V50.State.UmbrellaPending
		and now < V50.State.UmbrellaPendingUntil
	then
		V50.State.UmbrellaLastDecision = "PENDING"
		return false
	end
	...
	if V50.State.UmbrellaPending
		and now >= V50.State.UmbrellaPendingUntil
	then
		V50.State.UmbrellaPending = false
	end

	if now < V50.State.UmbrellaNextAttempt then
		V50.State.UmbrellaLastDecision = "BACKOFF"
		return false
	end

	if V50.countActiveUmbrellas() > 0 then
		V50.State.UmbrellaLastDecision = "ACTIVE"
		return false
	end
	...
	if not remote then
		V50.State.UmbrellaLastDecision = "NO REMOTE"
		return false
	end
...
function V50.getUmbrellaDirection(root)
	if not root then
		return Vector3.zero
	end
...
function V50.getUmbrellaRadiusAt(localTime)
	if localTime <= 0 then
		return 0
	end

	if localTime >= V50.C.UMBRELLA_GROW_TIME then
		return V50.C.UMBRELLA_DELETE_RADIUS
	end

	local alpha =
		math.clamp(
			localTime
				/ V50.C.UMBRELLA_GROW_TIME,
			0,
			1
		)

	-- Conservative growth model because only the tiny spawn size and later
	-- large deletion distances were observed, not the exact size curve.
	return
		V50.C.UMBRELLA_START_RADIUS
			+ (
				V50.C.UMBRELLA_DELETE_RADIUS
					- V50.C.UMBRELLA_START_RADIUS
			) * alpha
end

function V50.getUmbrellaCenter(
	rootPosition,
	baseDirection,
	side,
	time
	...
	local localTime =
		time - V50.C.UMBRELLA_ACTIVATION_DELAY

	if localTime < 0
		or localTime > V50.C.UMBRELLA_LIFETIME
	then
		return nil
	end
	...
	local travel =
		math.min(
			V50.C.UMBRELLA_MAX_DISTANCE,
			V50.C.UMBRELLA_SPEED * localTime
		)

	local direction =
		rotateY(
			baseDirection,
			V50.C.UMBRELLA_SPREAD_ANGLE
				* side
		)
	...
	local spawn =
		rootPosition
			- baseDirection
				* V50.C.UMBRELLA_SPAWN_BACK

	return spawn + direction * travel
end

function V50.makeUmbrellaThreatView(
	threats,
	rootPosition,
	baseDirection
)
	if not threats
		or not rootPosition
		or not baseDirection
		or baseDirection.Magnitude <= 0.01
	then
		return threats, 0
	end
				...
				if p0 and p1 then
					...
					for _, side in ipairs({-1, 1}) do
						...
						if u0 and u1 then
							local local0 =
								t0
									- V50.C.UMBRELLA_ACTIVATION_DELAY
	...
	-- Keep FarField unchanged on purpose. This makes the simulation
	-- conservative: umbrella benefit is only credited to exact tracked threats.
	view.FarField = threats.FarField
	view.UmbrellaDeletedAt = deletedAt

	return view, removed
end

function V50.useUmbrella()
	if not V50.canAttemptUmbrella() then
		return false
	end
	...
	local before =
		V50.countActiveUmbrellas()

	local now = os.clock()

	V50.State.UmbrellaPending = true
	V50.State.UmbrellaPendingUntil =
		now + V50.C.UMBRELLA_CONFIRM_TIMEOUT
	V50.State.UmbrellaLastDecision = "FIRE"
	...
	task.spawn(function()
		...
		while os.clock() - started
			< V50.C.UMBRELLA_CONFIRM_TIMEOUT
		do
			if V50.countActiveUmbrellas() > before then
				confirmed = true
				break
			end
		...
		V50.State.UmbrellaPending = false
		V50.State.UmbrellaPendingUntil = 0

		if confirmed then
			V50.State.UmbrellaFailStreak = 0
			V50.State.UmbrellaUseCount += 1
			V50.State.UmbrellaNextAttempt =
				os.clock()
					+ V50.C.UMBRELLA_CONFIRMED_GUARD
			V50.State.UmbrellaLastDecision =
				"CONFIRMED"
		else
			V50.State.UmbrellaFailStreak += 1

			local backoff =
				math.min(
					V50.C.UMBRELLA_FAILED_BACKOFF_MAX,
					V50.C.UMBRELLA_FAILED_BACKOFF_BASE
						* 2
							^ math.max(
								0,
								V50.State.UmbrellaFailStreak - 1
...
function V50.makeShieldThreatView(
	threats,
	center,
	startDelay,
	duration
...
local function constrainMoveAgainstBarriers(
	...
	then
		V50.State.WallConstraintState = "FREE"
	...
	V50.State.WallConstraintClearance =
	...
	V50.State.WallConstraintCount += 1

	if bestDirection then
		V50.State.WallConstraintState = "SLIDE"
		return bestDirection
	end

	V50.State.WallConstraintState = "STOP"
	return Vector3.zero
end
...
local function applyCurrentMove(
	...
	local requestedMove =
		V50.getExecutionDirection(root)
			or CurrentMove

	if requestedMove ~= CurrentMove
		and requestedMove then
		CurrentMove = requestedMove
	end
...
local function evaluateThreatSegment(
	...
	local minReturnClearance = math.huge
	local unstableGrazeBlock = false
	...
	local playerMove =
		flat(newPlayerPosition - oldPlayerPosition)
	...
	for _, threat in ipairs(activeThreats) do
		...
		local motionClass =
			threat.Data
				and threat.Data.MotionClass
				or "STRAIGHT"

		if motionClass == "RETURN" then
			minReturnClearance =
				math.min(
					minReturnClearance,
					clearance
			...
			if clearance
				< V50.C.RETURN_GRAZE_BLOCK_ZONE
			then
				unstableGrazeBlock = true
			end

			-- RETURN bullets are model-sensitive around their turning point.
			-- Keep a positive buffer even when the fitted arc says the path
			-- technically misses.
			local standoffClearance =
				clearance - V50.C.RETURN_STANDOFF

			if standoffClearance < 0 then
				local near =
					1 - math.clamp(
						(
							clearance
				...
				risk +=
					near * near
						* V50.C.RETURN_STANDOFF_WEIGHT
						* timeFactor
			end

			-- Do NOT repel from the projectile's absolute position.
			-- If a RETURN projectile is outside the player, absolute
			-- position repulsion can incorrectly push the bot inward/center.
			-- Instead, avoid crossing the predicted RETURN corridor.
			if clearance < V50.C.RETURN_CORRIDOR_ZONE then
				local tangent =
					flat(
						newProjectile
							- oldProjectile
				...
				if tangent.Magnitude > 0.01 then
					...
					local oldSide =
						tangent.X * oldOffset.Z
							- tangent.Z * oldOffset.X

					local newSide =
						tangent.X * newOffset.Z
							- tangent.Z * newOffset.X

					local preferredSide =
						threat.ReturnPreferredSide or 0

					local closeness =
						1 - math.clamp(
							clearance
								/ V50.C.RETURN_CORRIDOR_ZONE,
							0,
							1
						)

					if preferredSide ~= 0 then
						local signedOld =
							oldSide * preferredSide

						local signedNew =
							newSide * preferredSide
						...
						if shrink > 0 then
							risk +=
								shrink * shrink
									* V50.C.RETURN_SIDE_SHRINK_WEIGHT
									* (0.35 + closeness)
						end
					else
						if playerMove.Magnitude > 0.01 then
							local along =
								math.abs(
									playerMove.Unit:Dot(
										tangent
							...
							risk +=
								along * along
									* V50.C.RETURN_CENTERLINE_TANGENT_WEIGHT
									* (0.35 + closeness)
						end
					end
					...
					local distanceShrink =
						oldDistance - newDistance

					if distanceShrink > 0 then
						risk +=
							distanceShrink * distanceShrink
								* V50.C.RETURN_SIDE_SHRINK_WEIGHT
								* (0.25 + closeness)
					end
				end
			end
		elseif motionClass == "WAVE"
			and clearance
				< V50.C.RETURN_GRAZE_BLOCK_ZONE * 0.70
		then
			-- WAVE is also model-sensitive. Do not intentionally graze it.
			unstableGrazeBlock = true
		end
...
local function getNextInputs(previousDirection, depth, emergency)
	...
	local finePhase =
		not emergency
		and stepEnd <= V50.C.FINE_STEER_END
	...
	V50.State.FineSteerActive = finePhase
...
local function evaluateGrazeStep(
	...
	if unstableGrazeBlock
		or not grazePlanning
		or stepHits > 0
		or clearance == math.huge
		or clearance >= V50.C.GRAZE_MAX_CLEARANCE
	then
		return 0, 0
	end
	...
	if clearance <= V50.C.GRAZE_HARD_FLOOR then
		local danger =
			1 - math.clamp(
				clearance
					/ V50.C.GRAZE_HARD_FLOOR,
				0,
				1
			)

		return
			V50.C.GRAZE_TOO_CLOSE_PENALTY
				* (0.35 + danger * danger),
			0
	end

	local target =
		V50.C.GRAZE_TARGET_CLEARANCE

	local width =
		math.max(
			0.01,
			V50.C.GRAZE_MAX_CLEARANCE
				- target
		)
...
local function nodeSort(a, b)
	...
	if GRAZE_MODE
		and not V50.State.CornerActive
		and a.Hits == 0
		and b.Hits == 0
...
local function pruneBeamWithDiversity(
	...
	if depth < V50.C.DIVERSITY_START_DEPTH
		or keep < 4
	then
		return beam, 0
	end
...
local function reconstructDirections(node)
	local reverse = {}
	local cursor = node

	while cursor and cursor.Depth and cursor.Depth > 0 do
		reverse[#reverse + 1] =
			cursor.Dir or Vector3.zero
		cursor = cursor.Parent
	end
...
function V50.updateDenseSafety()
	local exact =
	...
	local critical =
		exact >= V50.C.DENSE_CRITICAL_COUNT
			or immediate >= V50.C.DENSE_CRITICAL_IMMEDIATE

	local active =
		critical
			or exact >= V50.C.DENSE_SAFE_COUNT
			or immediate >= V50.C.DENSE_SAFE_IMMEDIATE

	V50.State.DenseExactCount = exact
	V50.State.DenseImmediateCount = immediate
	V50.State.DenseSafetyActive = active
	V50.State.DenseCritical = critical

	if critical then
		V50.State.DenseRadiusBoost =
			V50.C.DENSE_CRITICAL_RADIUS_BOOST
	elseif active then
		V50.State.DenseRadiusBoost =
			V50.C.DENSE_RADIUS_BOOST
	else
		V50.State.DenseRadiusBoost = 0
	end

	V50.State.GrazeSuppressed =
		GRAZE_MODE and active
end
...
local function planPath(root, humanoid, threats, cornerDirection)
	...
	V50.State.DiversityInjected = 0
	V50.State.NearMissBest = 0
	V50.State.FineSteerActive = false
	V50.State.GrazeScoreBest = 0

	local grazePlanning =
		GRAZE_MODE
			and not V50.State.CornerActive
			and not V50.State.DenseSafetyActive

	V50.State.GrazePlanning = grazePlanning
		...
		for _, node in ipairs(beam) do
			...
			local emergency =
				...
				or node.MinClearance
					< emergencyClearance
				or node.MinReturnClearance
					< V50.C.RETURN_EMERGENCY_CLEARANCE
			local inputs =
				getNextInputs(
					node.Dir,
					depth,
	...
	table.sort(finalists, function(a, b)
		...
		then
			...
			local aReturn =
				a.Node.MinReturnClearance
					or math.huge
			local bReturn =
				b.Node.MinReturnClearance
					or math.huge

			if (
				aReturn < V50.C.RETURN_CORRIDOR_ZONE
				or bReturn < V50.C.RETURN_CORRIDOR_ZONE
			)
				and math.abs(aReturn - bReturn) > 0.08
			then
			...
			if CurrentMove.Magnitude > 0.01
				...
				and math.abs(costDelta)
					<= V50.C.ROBUST_CURRENT_MOVE_EPSILON
			then
				local aDot =
	...
	V50.State.NearMissBest =
		best.Node.NearMisses or 0

	V50.State.GrazeScoreBest =
		best.Node.GrazeScore or 0

	V50.State.ReturnMinClearance =
		best.Node.MinReturnClearance
			or math.huge

	V50.State.ReturnStandoffActive =
		V50.State.ReturnMinClearance
			< V50.C.RETURN_STANDOFF
	...
	return {
		...
		Directions =
			reconstructDirections(best.Node),
		Hits = best.Node.Hits,
		WallHits = best.Node.WallHits,
		FirstImpactTime =
			best.Node.FirstImpactTime
				or math.huge,
...
function V50.clearExecution()
	V50.State.ExecDirections = nil
	V50.State.ExecPositions = nil
	V50.State.ExecStartedAt = 0
	V50.State.ExecMaxAge = 0
	V50.State.ExecState = "FREE"
	V50.State.ExecStep = 0
	V50.State.ExecPositionError = 0
end

function V50.startExecution(plan)
	if not plan
		or not plan.Directions
		or #plan.Directions == 0
	then
		V50.clearExecution()
		return
	end
	...
	local maxAge =
		V50.C.MPC_EXEC_NORMAL_AGE

	if V50.State.DenseCritical then
		maxAge =
			V50.C.DENSE_CRITICAL_EXEC_AGE
	elseif V50.State.DenseSafetyActive then
		maxAge =
			V50.C.DENSE_EXEC_AGE
	elseif plan.Hits > 0 then
		maxAge =
			V50.C.MPC_EXEC_RESCUE_AGE
	elseif GRAZE_MODE
		and V50.State.GrazePlanning
	then
		maxAge =
			V50.C.MPC_EXEC_GRAZE_AGE
	elseif V50.State.CornerActive then
		maxAge =
			V50.C.MPC_EXEC_CORNER_AGE
	end

	V50.State.ExecDirections = plan.Directions
	V50.State.ExecPositions = plan.Path
	V50.State.ExecStartedAt = os.clock()
	V50.State.ExecMaxAge = maxAge
	V50.State.ExecState = "ARMED"
	V50.State.ExecStep = 1
	V50.State.ExecCount += 1
	V50.State.ExecPositionError = 0
end

function V50.getExecutionStep(elapsed)
	for stepIndex = 1, PLAN_STEPS do
		if elapsed
			< getStepEndTime(stepIndex)
		then
...
function V50.getExecutionDirection(root)
	local directions =
		V50.State.ExecDirections
	...
	local elapsed =
		os.clock()
			- V50.State.ExecStartedAt

	local liveMaxAge =
		V50.State.ExecMaxAge
	...
	V50.State.ExecStep = stepIndex

	-- Compare the real player position with the MPC position for the same
	-- point in time. If physics/wall interaction has pushed us far away from
	-- the planned trajectory, stop replaying stale controls.
	local positions =
		V50.State.ExecPositions

	if positions and positions[stepIndex] then
		local errorDistance =
			flat(
				root.Position
					- positions[stepIndex]
		...
		V50.State.ExecPositionError =
			errorDistance

		if errorDistance
			> V50.C.MPC_EXEC_POSITION_ERROR
		then
			V50.clearExecution()
			return nil
		end
	...
	-- Short physical wall sanity check. This does not choose a new dodge;
	-- it only cancels a stale MPC action if reality no longer matches it.
	if direction.Magnitude > 0.01 then
		local projected =
			root.Position
				+ direction.Unit
					* 16
					* V50.C.MPC_EXEC_WALL_CHECK_TIME
	...
	V50.State.ExecState = "FOLLOW"

	return direction
end

function V50.hasExecution()
	if V50.State.ExecDirections == nil then
		return false
	end
...
Main.Size = UDim2.fromOffset(260, 447)
...
Info.Size = UDim2.new(1, -20, 0, 194)
...
Toggle.Position = UDim2.fromOffset(10, 252)
...
GrazeToggle.Position = UDim2.fromOffset(10, 297)
...
RadiusLabel.Position = UDim2.fromOffset(42, 338)
...
Minus.Position = UDim2.fromOffset(10, 338)
...
Plus.Position = UDim2.new(1, -38, 0, 338)
...
PathToggle.Position = UDim2.fromOffset(10, 379)
...
Footer.Position = UDim2.fromOffset(10, 418)
Footer.BackgroundTransparency = 1
Footer.Text = "v50 / Washukitsu Cleaner + Charge Fix + MPC"
...
Toggle.Activated:Connect(function()
	...
	if ENABLED then
		...
		V50.State.CornerActive = false
		V50.State.CornerDirection = nil
		V50.State.ShieldPlanHoldUntil = 0
		V50.State.ShieldLastDecision = "IDLE"
		V50.State.ShieldCrisisReason = "-"
		V50.State.ShieldPending = false
		V50.State.ShieldPendingUntil = 0
		V50.State.ShieldRetryCount = 0
		V50.State.UmbrellaPending = false
		V50.State.UmbrellaPendingUntil = 0
		V50.State.UmbrellaLastDecision = "IDLE"
		V50.State.WallConstraintState = "FREE"
		V50.State.ReturnMinClearance = math.huge
		V50.State.ReturnStandoffActive = false
		V50.clearExecution()
		V50.State.DenseSafetyActive = false
		V50.State.DenseCritical = false
		V50.State.DenseRadiusBoost = 0
		V50.State.GrazeSuppressed = false
		clearPathVisual()
		enableControls()
...
local function decisionStep()
	...
	if now < V50.State.ShieldPlanHoldUntil then
	...
	local ok, err = pcall(function()
		...
		V50.updateDenseSafety()

		local activeShield =
			V50.getActiveShieldModel()
		...
		local needPlan =
			...
			or (
				V50.State.DenseSafetyActive
				and baseline.MinClearance
					<= V50.C.DENSE_FORCE_REPLAN_CLEARANCE
			)
		...
		if not needPlan then
			...
			if V50.State.DenseSafetyActive then
				V50.clearExecution()
			elseif V50.hasExecution() then
				LastMode = "MPC EXEC"
				applyCurrentMove(
					humanoid,
					root
		...
		local umbrellaRequested = false

		local umbrellaPanic =
			plan.Hits > 0
				and (
					plan.FirstImpactTime
						or math.huge
				) <= V50.C.UMBRELLA_SHIELD_PANIC_TIME

		local umbrellaCrisis =
			not umbrellaPanic
				and (
					plan.Hits > 0
					or (
					...
					or (
						plan.MinClearance
							<= V50.C.UMBRELLA_FRAGILE_CLEARANCE
						and plan.EscapeOptions <= 1
					)
				)

		if umbrellaCrisis
			and V50.canAttemptUmbrella()
		then
			local umbrellaDirection =
				V50.getUmbrellaDirection(root)

			if umbrellaDirection.Magnitude > 0.01 then
				local simStarted = os.clock()

				local umbrellaThreats,
					removed =
					V50.makeUmbrellaThreatView(
						effectiveThreats,
						root.Position,
						umbrellaDirection
					)

				V50.State.UmbrellaLastRemoved =
					removed

				local umbrellaPlan = nil

				if removed
					>= V50.C.UMBRELLA_MIN_REMOVED
				then
					umbrellaPlan =
						planPath(
							root,
				...
				V50.State.UmbrellaSimMs =
					(os.clock() - simStarted)
						* 1000

				local hitGain =
					umbrellaPlan
						and umbrellaPlan.Hits
							< plan.Hits

				local clearanceGain =
					umbrellaPlan
						and (
							(umbrellaPlan.MinClearance
								or -math.huge)
							- (plan.MinClearance
								or -math.huge)
						)
				...
				local escapeGain =
					umbrellaPlan
						and (
							(umbrellaPlan.EscapeOptions or 0)
				...
				local makesSafe =
					umbrellaPlan
						and umbrellaPlan.Hits == 0
						and umbrellaPlan.WallHits == 0
				...
				local useful =
					...
					and (
						...
						or (
							...
							and (
								clearanceGain
									>= V50.C.UMBRELLA_MIN_CLEARANCE_GAIN
								or escapeGain
									>= V50.C.UMBRELLA_ESCAPE_GAIN
							)
						)
					)

				if useful then
					if V50.useUmbrella() then
						umbrellaRequested = true
						V50.State.UmbrellaLastDecision =
							hitGain
								and "RESCUE FIRE"
								or "CLEAR FIRE"
		...
		then
			...
			local deferShieldForUmbrella =
				umbrellaRequested
					and (
						not unavoidable
						or firstImpact
							> V50.C.SHIELD_PANIC_IMPACT
					)

			local imminentPlanHit =
				unavoidable
				and firstImpact
					<= V50.C.SHIELD_PANIC_IMPACT
			...
			local urgentPlanHit =
				unavoidable
				and firstImpact
					<= V50.C.SHIELD_TRIGGER_IMPACT

			local fragilePlan =
				plan.Hits == 0
				and plan.MinClearance
					<= V50.C.SHIELD_FRAGILE_CLEARANCE
				and plan.EscapeOptions
					<= V50.C.SHIELD_FRAGILE_ESCAPE_OPTIONS

			local trappedPlan =
				V50.State.CornerActive
				and plan.EscapeOptions
					<= V50.C.SHIELD_FRAGILE_ESCAPE_OPTIONS

			local baselineCrisis =
				baseline.Hits > 0
				and baseline.EarliestImpact
					<= V50.C.SHIELD_PANIC_IMPACT
				and (
			...
			local denseCrisis =
				V50.State.DenseSafetyActive
				and (
					V50.State.DenseImmediateCount
						>= V50.C.DENSE_SHIELD_IMMEDIATE
					or plan.MinClearance
						<= V50.C.DENSE_SHIELD_CLEARANCE
				)
				and plan.EscapeOptions
					<= V50.C.SHIELD_FRAGILE_ESCAPE_OPTIONS
			...
			local crisis =
				...
				and not deferShieldForUmbrella

			if crisis then
				if imminentPlanHit then
					V50.State.ShieldCrisisReason = "PANIC"
				elseif urgentPlanHit then
					V50.State.ShieldCrisisReason = "URGENT"
				elseif unavoidable then
					V50.State.ShieldCrisisReason = "UNAVOIDABLE"
				elseif trappedPlan then
					V50.State.ShieldCrisisReason = "TRAPPED"
				elseif fragilePlan then
					V50.State.ShieldCrisisReason = "FRAGILE"
				elseif denseCrisis then
					V50.State.ShieldCrisisReason = "DENSE"
				else
					V50.State.ShieldCrisisReason = "BASELINE"
				end
				...
				V50.State.ShieldSimMs =
					(os.clock() - simStarted)
						* 1000

				local shieldImprovesHits =
					shieldPlan
						and shieldPlan.Hits
							< plan.Hits

				local shieldMakesSafe =
					shieldPlan
				...
				local clearanceGain =
					shieldPlan
						and (
							(shieldPlan.MinClearance or -math.huge)
							- (plan.MinClearance or -math.huge)
						)
						or 0
				...
				local escapeGain =
					shieldPlan
						and (
							(shieldPlan.EscapeOptions or 0)
							- (plan.EscapeOptions or 0)
						)
						or 0

				local materiallySafer =
					shieldMakesSafe
						and (
							unavoidable
							or denseCrisis
							or clearanceGain
								>= V50.C.SHIELD_CLEARANCE_GAIN
							or escapeGain
								>= V50.C.SHIELD_ESCAPE_GAIN
						)

				-- PANIC path:
				-- if the chosen route itself predicts impact in <= 0.38s
				-- and the shield actually intercepts a relevant projectile,
				-- do not wait for the second MPC pass to be "perfect".
				-- The resource is specifically for this case.
				local panicCast =
					imminentPlanHit
						and removed
							>= V50.C.SHIELD_MIN_REMOVED
				...
				local shouldCast =
					panicCast
						or shieldImprovesHits
						or materiallySafer

				if shouldCast then
					if V50.useShield(root) then
						if shieldPlan then
							plan = shieldPlan
						end
						...
						if panicCast then
							V50.State.ShieldLastDecision =
								"PANIC FIRE"
						elseif shieldImprovesHits then
							V50.State.ShieldLastDecision =
								"RESCUE"
						else
							V50.State.ShieldLastDecision =
		...
		elseif activeShield then
			V50.State.ShieldCrisisReason = "ACTIVE"
			V50.State.ShieldLastDecision =
				"ACTIVE"
		else
			-- canUseShield() already writes NO CHARGE / COOLDOWN / etc.
			if plan.Hits == 0 then
				V50.State.ShieldCrisisReason = "-"
			end
		end
		...
		if shieldUsed then
			if V50.State.ShieldLastDecision == "PANIC FIRE" then
				LastMode = "SHIELD PANIC"
			elseif V50.State.ShieldLastDecision == "RESCUE" then
				LastMode = "SHIELD RESCUE"
			else
				LastMode = "SHIELD BREAKOUT"
			end
		elseif umbrellaRequested then
			LastMode = "UMBRELLA CLEAR"
		elseif V50.State.DenseCritical then
			LastMode = "DENSE CRITICAL"
		elseif V50.State.DenseSafetyActive then
			LastMode = "DENSE SAFE"
		elseif GRAZE_MODE
			and V50.State.GrazePlanning
		...
		Info.Text =
			...
			.. (
				V50.getActiveShieldModel()
					and "ACTIVE"
					or (
						V50.getShieldCooldownRemaining() > 0
							and (
								"CD "
								.. string.format(
									"%.0f",
									V50.getShieldCooldownRemaining()
								)
							)
							or V50.State.ShieldLastDecision
					)
			)
			.. " / "
			.. V50.State.ShieldCrisisReason
			.. (
				V50.State.ShieldPending
					and (
						" / PENDING"
						.. (
							V50.State.ShieldRetryCount > 0
			...
			.. " / Del "
			.. tostring(V50.State.ShieldLastRemoved)
			.. " / Sim "
			.. string.format(
				"%.1fms",
				V50.State.ShieldSimMs
			...
			.. " / #"
			.. tostring(
				V50.State.UmbrellaUseCount
			)
			.. "\nSearch D"
			.. tostring(V50.State.DiversityInjected)
			.. " / NM "
			.. tostring(V50.State.NearMissBest)
			.. " / Fine "
			.. (
				V50.State.FineSteerActive
					and "ON"
					or "-"
			)
			.. "\nGraze "
			...
			.. " / Fix "
			.. tostring(
				V50.State.WallConstraintCount
			)
			.. "\nTraj S"
			.. tostring(
				V50.State.TrajectoryStraight
			)
			.. " C"
			.. tostring(
				V50.State.TrajectoryCurve
			)
			.. " W"
			.. tostring(
				V50.State.TrajectoryWave
			)
			.. " R"
			.. tostring(
				V50.State.TrajectoryReturn
			)
			.. "\nRetPath "
			.. (
				V50.State.ReturnMinClearance
