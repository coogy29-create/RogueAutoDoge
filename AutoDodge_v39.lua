...
function V39.directionOpenDistance(position, direction)
	local nearest = V39.C.CORNER_PROBE_DISTANCE
...
function V39.bestCornerExit(position)
	local bestDirection = nil
	local bestScore = -math.huge

	for i = 0, V39.C.CORNER_PROBE_DIRECTIONS - 1 do
		local angle =
			i * math.pi * 2
				/ V39.C.CORNER_PROBE_DIRECTIONS
...
function V39.updateCornerObjective(position)
	local _, minimumOpen, closeRays =
		barrierOpenness(position)

	-- Once we have actually left the corner, release the objective.
	if minimumOpen >= V39.C.CORNER_RELEASE_OPEN
		and closeRays < V39.C.CORNER_CLOSE_RAYS
	then
		V39.State.CornerActive = false
		V39.State.CornerDirection = nil
		return nil
	end

	local trapped =
		minimumOpen < V39.C.CORNER_MIN_OPEN
		and closeRays >= V39.C.CORNER_CLOSE_RAYS

	-- Critical anti-wiggle behavior:
	-- keep the SAME exit direction as long as that channel still exists.
	if V39.State.CornerActive
		and V39.State.CornerDirection
	then
		local clearance =
			V39.directionOpenDistance(
				position,
				V39.State.CornerDirection
			)

		if clearance
			>= V39.C.CORNER_EXIT_MIN_CLEARANCE
		then
			return V39.State.CornerDirection
		end
	...
	local direction =
		V39.bestCornerExit(position)

	if direction then
		V39.State.CornerActive = true
		V39.State.CornerDirection = direction
	end

	return direction
end

function V39.cornerRouteCost(
	oldPosition,
	newPosition,
	direction,
	cornerDirection
...
function V39.getShieldCooldownRemaining()
	return math.max(
		0,
		V39.C.SHIELD_COOLDOWN
			- (os.clock() - V39.State.ShieldLastUse)
	)
end

function V39.findShieldPart()
	local core = workspace:FindFirstChild("Core__Game")
	if not core then
		return nil
	end
...
function V39.getActiveShieldModel()
	local now = os.clock()
	local shield = V39.findShieldPart()

	if shield then
		if V39.State.ShieldPart ~= shield then
			V39.State.ShieldPart = shield
			V39.State.ShieldCenter = shield.Position
			V39.State.ShieldUntil =
				now + V39.C.SHIELD_DURATION

			-- Also catches a manual shield use while the bot is running.
			if now - V39.State.ShieldLastUse > 1.0 then
				V39.State.ShieldLastUse = now
			end
		else
			V39.State.ShieldCenter = shield.Position
		end
		...
		local remaining =
			math.max(
				0,
				V39.State.ShieldUntil - now
			)

		if remaining > 0 then
			return {
				Center = V39.State.ShieldCenter,
				StartDelay = 0,
				Duration = remaining
			}
		end
	...
	if V39.State.ShieldPart ~= nil then
		V39.State.ShieldPart = nil
		V39.State.ShieldCenter = nil
		V39.State.ShieldUntil = 0
	end

	return nil
end

function V39.canUseShield()
	local charge =
		select(1, V39.getShieldCharge())

	if charge < V39.C.SHIELD_REQUIRED_CHARGE then
		V39.State.ShieldLastDecision = "NO CHARGE"
		return false
	end

	if V39.getShieldCooldownRemaining() > 0 then
		V39.State.ShieldLastDecision = "COOLDOWN"
		return false
	end

	if V39.getActiveShieldModel() then
		V39.State.ShieldLastDecision = "ACTIVE"
		return false
	end
	...
	if not event or not event:IsA("RemoteEvent") then
		V39.State.ShieldLastDecision = "NO REMOTE"
		return false
	end
...
function V39.useShield(root)
	if not root or not root.Parent then
		return false
	end

	if not V39.canUseShield() then
		return false
	end
	...
	V39.State.ShieldLastUse = now
	V39.State.ShieldCenter = root.Position
	V39.State.ShieldUntil =
		now
			+ V39.C.SHIELD_ACTIVATION_DELAY
			+ V39.C.SHIELD_DURATION

	V39.State.ShieldPlanHoldUntil =
		now + V39.C.SHIELD_PLAN_HOLD

	V39.State.ShieldUseCount += 1
	V39.State.ShieldLastDecision = "FIRED"

	event:FireServer(5, true)

	task.delay(
		V39.C.SHIELD_PRESS_TIME,
		function()
...
function V39.makeShieldThreatView(
	threats,
	center,
	startDelay,
	duration
...
local function getNextInputs(previousDirection, depth, emergency)
	...
	local finePhase =
		not emergency
		and stepEnd <= V39.C.FINE_STEER_END
	...
	V39.State.FineSteerActive = finePhase
...
local function evaluateGrazeStep(
	...
	if not grazePlanning
		...
		or clearance >= V39.C.GRAZE_MAX_CLEARANCE
	then
		return 0, 0
	end
	...
	if clearance <= V39.C.GRAZE_HARD_FLOOR then
		local danger =
			1 - math.clamp(
				clearance
					/ V39.C.GRAZE_HARD_FLOOR,
				0,
				1
			)

		return
			V39.C.GRAZE_TOO_CLOSE_PENALTY
				* (0.35 + danger * danger),
			0
	end

	local target =
		V39.C.GRAZE_TARGET_CLEARANCE

	local width =
		math.max(
			0.01,
			V39.C.GRAZE_MAX_CLEARANCE
				- target
		)
...
local function nodeSort(a, b)
	...
	if GRAZE_MODE
		and not V39.State.CornerActive
		and a.Hits == 0
		and b.Hits == 0
...
local function pruneBeamWithDiversity(
	...
	if depth < V39.C.DIVERSITY_START_DEPTH
		or keep < 4
	then
		return beam, 0
	end
...
local function planPath(root, humanoid, threats, cornerDirection)
	...
	V39.State.DiversityInjected = 0
	V39.State.NearMissBest = 0
	V39.State.FineSteerActive = false
	V39.State.GrazeScoreBest = 0

	local grazePlanning =
		GRAZE_MODE
			and not V39.State.CornerActive

	V39.State.GrazePlanning = grazePlanning
		...
		then
			...
			if CurrentMove.Magnitude > 0.01
				...
				and math.abs(costDelta)
					<= V39.C.ROBUST_CURRENT_MOVE_EPSILON
			then
				local aDot =
	...
	V39.State.NearMissBest =
		best.Node.NearMisses or 0

	V39.State.GrazeScoreBest =
...
Main.Size = UDim2.fromOffset(260, 363)
...
Toggle.Position = UDim2.fromOffset(10, 168)
...
GrazeToggle.Position = UDim2.fromOffset(10, 213)
...
RadiusLabel.Position = UDim2.fromOffset(42, 254)
...
Minus.Position = UDim2.fromOffset(10, 254)
...
Plus.Position = UDim2.new(1, -38, 0, 254)
...
PathToggle.Position = UDim2.fromOffset(10, 295)
...
Footer.Position = UDim2.fromOffset(10, 334)
Footer.BackgroundTransparency = 1
Footer.Text = "v39 / Robust Beam + Graze Mode + Smart Shield"
...
Toggle.Activated:Connect(function()
	...
	if ENABLED then
		...
		V39.State.CornerActive = false
		V39.State.CornerDirection = nil
		V39.State.ShieldPlanHoldUntil = 0
		V39.State.ShieldLastDecision = "IDLE"
		clearPathVisual()
		enableControls()
...
local function decisionStep()
	...
	if now < V39.State.ShieldPlanHoldUntil then
	...
	local ok, err = pcall(function()
		...
		if not activeShield
			and plan.Hits > 0
			and V39.canUseShield()
		then
			local urgentEnough =
				baseline.EarliestImpact ~= math.huge
				and baseline.EarliestImpact
					<= V39.C.SHIELD_TRIGGER_IMPACT

			local cornerEmergency =
				V39.State.CornerActive
			...
			if urgentEnough or cornerEmergency then
				...
				then
					if V39.useShield(root) then
						plan = shieldPlan
						shieldUsed = true
						V39.State.ShieldLastDecision =
							"BREAKOUT"
					end
				else
					V39.State.ShieldLastDecision =
		...
		elseif plan.Hits > 0 then
			LastMode = "RESCUE"
		elseif V39.State.CornerActive then
		...
		Info.Text =
			...
			.. (
				V39.getActiveShieldModel()
					and "ACTIVE"
					or (
						V39.getShieldCooldownRemaining() > 0
							and (
								"CD "
								.. string.format(
									"%.0f",
									V39.getShieldCooldownRemaining()
								)
							)
							or V39.State.ShieldLastDecision
					)
			)
			.. " / Del "
			.. tostring(V39.State.ShieldLastRemoved)
			.. " / Sim "
			.. string.format(
				"%.1fms",
				V39.State.ShieldSimMs
			...
			.. "\nSearch D"
			.. tostring(V39.State.DiversityInjected)
			.. " / NM "
			.. tostring(V39.State.NearMissBest)
			.. " / Fine "
			.. (
				V39.State.FineSteerActive
					and "ON"
					or "-"
			)
			.. "\nGraze "
