...
local V32 = {
	C = {
		...
		SHAPE_EXACT_HORIZON = 0.85,
		EXACT_STEP_CAP_IMMEDIATE = 28,
		EXACT_STEP_CAP_NEAR = 20,
		WALL_GUARD_LOOKAHEAD = 0.14,
		WALL_GUARD_MIN_DISTANCE = 1.35,
		WALL_GUARD_TURN_ANGLES = {30, -30, 55, -55, 80, -80, 105, -105},
...
function V32.getObjectBox(obj, part)
...
function V32.classifyGeometry(size)
	if not size then
		return "Circle"
	end
	...
	if shortAxis >= V32.C.OBB_MIN_SHORT_AXIS
		or area >= V32.C.OBB_MIN_AREA
		or longAxis >= V32.C.OBB_FORCE_LONG_AXIS
	then
		return "OBB"
	end

	-- Narrow elongated bullets use an oriented capsule.
	if aspect >= V32.C.CAPSULE_MIN_ASPECT
		and longAxis <= V32.C.CAPSULE_MAX_LONG_AXIS
	then
		return "Capsule"
	end
...
function V32.getGeometryRadius(size)
	if not size then
		return 0.5
	end
...
function V32.getBroadRadius(size)
	if not size then
		return 0.5
	end
...
function V32.getLongAxis(cf, size)
...
function V32.segmentMinDistance2D(relativeA, relativeB)
...
function V32.pointSegmentDistance2D(point, a, b)
	local p = flat(point)
	local aa = flat(a)
	local bb = flat(b)
	local d = bb - aa
...
function V32.pointBoxSignedDistance2D(boxCf, boxSize, point, expansion)
...
function V32.getCapsuleSegment(cf, size, expansion)
	if not cf or not size then
		return nil, nil, 0
	end
	...
	local axis = V32.getLongAxis(cf, size)
...
function V32.capsuleSignedDistance2D(cf, size, point, expansion)
	local a, b, radius =
		V32.getCapsuleSegment(
			cf,
			size,
			expansion
		)
...
function V32.predictGeometryFrame(data, predictedPosition, t)
...
function V32.refreshGeometryTracking(data, obj, part, dt)
	local now = os.clock()
	...
	data.NextGeometryRefresh =
		now + V32.C.GEOMETRY_REFRESH

	local boxCf, boxSize =
		V32.getObjectBox(obj, part)
	...
	local previousAxis =
		data.ShapeAxis
			or V32.getLongAxis(boxCf, boxSize)

	local newAxis =
		V32.getLongAxis(boxCf, boxSize)
	...
	if dt and dt > 0 then
		...
		data.GeometryVelocity =
			(data.GeometryVelocity or rawVelocity):Lerp(
				rawVelocity,
				V32.C.GEOMETRY_VELOCITY_SMOOTH
			)
	...
	data.ShapeType =
		V32.classifyGeometry(boxSize)
	data.ShapeRadius =
		V32.getGeometryRadius(boxSize)
	data.BroadRadius =
		V32.getBroadRadius(boxSize)
	data.ShapeAxis = newAxis
	data.LastBoxPosition = boxCf.Position
end

function V32.shapeSignedDistance(
	shapeType,
	cf,
	size,
	point,
...
function V32.evaluateCapsuleSweep(
	oldCf,
	newCf,
	size,
	oldPlayer,
...
function V32.directionBarrierClearance(origin, direction, maxDistance)
...
function V32.positionIndexNearTime(targetTime)
	local bestIndex = 1
...
function V32.analyzeSlowFan(root, humanoid, threats)
	local sampleIndex =
		V32.positionIndexNearTime(
			V32.C.FAN_SAMPLE_TIME
		)
	...
	local occupied =
		table.create(V32.C.FAN_BINS, false)

	local binAngle =
		math.pi * 2 / V32.C.FAN_BINS
	...
	for _, threat in ipairs(threats) do
		...
		then
			...
			if future and current then
				...
				if distance >= 0.8
					and distance
						<= V32.C.FAN_MAX_DISTANCE
					and approach
						>= V32.C.FAN_MIN_APPROACH_DOT
				then
					count += 1
					centroid += future
					nearest =
	...
	if count < V32.C.FAN_MIN_PROJECTILES then
		if os.clock()
			<= V32.State.FanGraceUntil
			and V32.State.FanDirection
		then
			return {
				PreferredDirection =
					V32.State.FanDirection,
				Count = V32.State.FanCount,
				ArcDegrees = V32.State.FanArc,
				TimeToEdge =
					V32.State.FanEdgeTime,
				ClosingTime =
					V32.State.FanClosingTime
			}
		end

		V32.State.FanCount = 0
		V32.State.FanArc = 0
		V32.State.FanSide = "-"
		V32.State.FanDirection = nil
		return nil
	end
	...
	for i = 1, V32.C.FAN_BINS * 2 do
		local bin =
			((i - 1) % V32.C.FAN_BINS) + 1
	...
	local arcDegrees =
		longest * 360 / V32.C.FAN_BINS

	if arcDegrees < V32.C.FAN_MIN_ARC
		or arcDegrees > V32.C.FAN_MAX_ARC
	then
		return nil
	end
	...
	V32.State.FanCount = count
	V32.State.FanArc = arcDegrees
	V32.State.FanSide = side
	V32.State.FanDirection = preferred
	V32.State.FanGraceUntil =
		os.clock() + V32.C.FAN_GRACE
	V32.State.FanEdgeTime = timeToEdge
	V32.State.FanClosingTime = closingTime
...
function V32.fanCost(
	...
	local required =
		walkSpeed
			...
			* V32.C.FAN_PROGRESS_FRACTION

	if progress < required then
		cost +=
			(required - progress)
				* V32.C.FAN_PROGRESS_WEIGHT
	end
...
function V32.cornerStepCost(
	...
	then
		...
		cost +=
			approach
				* V32.C.CORNER_APPROACH_WEIGHT
				* nearFactor
	end

	if newClearance
		< V32.C.CORNER_CRITICAL_DISTANCE
	then
		local depth =
			V32.C.CORNER_CRITICAL_DISTANCE
				- newClearance
		...
		cost +=
			depth * depth
				* V32.C.CORNER_CRITICAL_WEIGHT
	end

	return cost
end

function V32.cornerFinalCost(
	rootAverage,
	rootMinimum,
	averageOpen,
	minimumOpen,
	...
	if averageOpen < rootAverage then
		cost +=
			(rootAverage - averageOpen)
				* V32.C.CORNER_OPEN_LOSS_WEIGHT
	end

	if minimumOpen < rootMinimum then
		cost +=
			(rootMinimum - minimumOpen)
				* V32.C.CORNER_MIN_OPEN_LOSS_WEIGHT
	end

	cost +=
		closeRays
			* V32.C.CORNER_CLOSE_RAY_EXTRA
...
RunService.Heartbeat:Connect(function(dt)
	...
	for obj, data in pairs(ProjectileData) do
		...
		if not part or not part.Parent then
			...
			data.Samples = 0
			V32.refreshGeometryTracking(data, obj, part, dt)
...
local function buildThreatSnapshot(root, humanoid)
			...
			do
				...
				then
					...
					local exactShape =
						...
						or (
							shapeType ~= "Circle"
							and t <= V32.C.SHAPE_EXACT_HORIZON
						)
	...
	for stepIndex = 1, TOTAL_PREDICTION_STEPS do
		...
		local cap =
			t <= IMMEDIATE_THREAT_HORIZON
				and V32.C.EXACT_STEP_CAP_IMMEDIATE
				or V32.C.EXACT_STEP_CAP_NEAR
		...
		if rawCount > cap then
			...
			for i = cap + 1, rawCount do
				local overflow = bucket[i]
				local p =
					overflow.Positions
	...
	V32.State.ExactRawPeak = rawPeak
	V32.State.ExactKeptPeak = keptPeak
	...
	V32.State.CircleThreats = circleCount
	V32.State.CapsuleThreats = capsuleCount
	V32.State.OBBThreats = obbCount
...
Footer.Text = "v32 / V4 Brain / Wall Guard + Geometry Budget + Fan"
...
local function wallGuardMove(root, humanoid, desiredMove)
	...
	local probeDistance =
		math.max(
			V32.C.WALL_GUARD_MIN_DISTANCE,
			humanoid.WalkSpeed
				* V32.C.WALL_GUARD_LOOKAHEAD
		)
	...
	for _, degrees in ipairs(V32.C.WALL_GUARD_TURN_ANGLES) do
		local candidate =
...
local function getPlanInterval()
	...
	then
		base = PLAN_INTERVAL_URGENT
	elseif LastThreatCount > 0 then
		base = PLAN_INTERVAL_ACTIVE
	else
		base = PLAN_INTERVAL_IDLE
	end

	if LastPlanMs >= V32.C.PLAN_BACKOFF_HARD_MS then
		return math.max(
			base,
			V32.C.PLAN_BACKOFF_HARD_INTERVAL
		)
	end

	if LastPlanMs >= V32.C.PLAN_BACKOFF_SOFT_MS then
		return math.max(
			base,
			V32.C.PLAN_BACKOFF_SOFT_INTERVAL
		)
	end
...
local function decisionStep()
	...
	local ok, err = pcall(function()
		...
		if #threats == 0 then
			V32.State.CircleThreats = 0
			V32.State.CapsuleThreats = 0
			V32.State.OBBThreats = 0
			V32.State.FanCount = 0
			V32.State.FanArc = 0
			V32.State.FanSide = "-"
			V32.State.FanDirection = nil
...
task.spawn(function()
	while Gui.Parent do
		...
		Info.Text =
			...
			.. "\nGeo C"
			.. tostring(V32.State.CircleThreats)
			.. " K"
			.. tostring(V32.State.CapsuleThreats)
			.. " B"
			.. tostring(V32.State.OBBThreats)
			.. "\nX "
			.. tostring(V32.State.ExactRawPeak)
			.. ">"
			.. tostring(V32.State.ExactKeptPeak)
			.. " / W "
			.. (
				V32.State.WallGuard
					and "GUARD"
