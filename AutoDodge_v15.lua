...
local FrameDtEMA = C.FRAME_TARGET_DT
local ImmediateMsEMA = 0
local LastPlannerSliceBudgetMs = C.PLANNER_SLICE_BUDGET * 1000
...
RunService.Heartbeat:Connect(function(dt)
	...
	FrameDtEMA +=
		(dt - FrameDtEMA) * C.FRAME_DT_EMA_ALPHA
...
local function updateFanModel(detectedFront, root, humanoid, threats, now)
	if not detectedFront then
		if CurrentFanInfo
			and now - LastFanSeenAt <= C.FAN_INFO_GRACE
		then
			local age =
				math.clamp(
					(now - LastFanSeenAt) / C.FAN_INFO_GRACE,
					0,
					1
				)
	...
	LastFanForecastTime = detectedFront.ForecastTime or C.FAN_SAMPLE_TIME
...
local function getFanInfo(now)
	...
	now = now or os.clock()
	if now - LastFanSeenAt > C.FAN_INFO_GRACE then
		return nil
	end
...
local function runImmediateMPC()
	...
	local now = os.clock()
	if now - LastImmediateMPCTime < C.IMMEDIATE_MPC_INTERVAL then
		return
	end
...
Footer.Text = "v15 / Spatial MPC / Native Focus"
...
local function decisionStep()
	if not C.ENABLED or planning then
		return
	end
