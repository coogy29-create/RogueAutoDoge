...
function V47.directionOpenDistance(position, direction)
	local nearest = V47.C.CORNER_PROBE_DISTANCE
...
function V47.bestCornerExit(position)
	local bestDirection = nil
	local bestScore = -math.huge

	for i = 0, V47.C.CORNER_PROBE_DIRECTIONS - 1 do
		local angle =
			i * math.pi * 2
				/ V47.C.CORNER_PROBE_DIRECTIONS
...
function V47.updateCornerObjective(position)
	local _, minimumOpen, closeRays =
		barrierOpenness(position)

	-- Once we have actually left the corner, release the objective.
	if minimumOpen >= V47.C.CORNER_RELEASE_OPEN
		and closeRays < V47.C.CORNER_CLOSE_RAYS
	then
		V47.State.CornerActive = false
		V47.State.CornerDirection = nil
		return nil
	end

	local trapped =
		minimumOpen < V47.C.CORNER_MIN_OPEN
		and closeRays >= V47.C.CORNER_CLOSE_RAYS

	-- Critical anti-wiggle behavior:
	-- keep the SAME exit direction as long as that channel still exists.
	if V47.State.CornerActive
		and V47.State.CornerDirection
	then
		local clearance =
			V47.directionOpenDistance(
				position,
				V47.State.CornerDirection
			)

		if clearance
			>= V47.C.CORNER_EXIT_MIN_CLEARANCE
		then
			return V47.State.CornerDirection
		end
	...
	local direction =
		V47.bestCornerExit(position)

	if direction then
		V47.State.CornerActive = true
		V47.State.CornerDirection = direction
	end

	return direction
end

function V47.cornerRouteCost(
	oldPosition,
	newPosition,
	direction,
	cornerDirection
...
function V47.pushMotionHistory(data, position, now)
	...
	while #history > V47.C.RETURN_HISTORY_MAX do
		table.remove(history, 1)
	end
end

function V47.fitReturnArc(data)
	local history = data.MotionHistory
	...
	if radius < V47.C.RETURN_MIN_RADIUS
		or radius > V47.C.RETURN_MAX_RADIUS
	then
		return nil
	end
...
function V47.predictReturnArcPoint(data, t, origin)
	...
	local angle =
		rate
			* math.min(
				t,
				V47.C.RETURN_MAX_PREDICTION_TIME
			)
...
function V47.updateReturnModel(
	data,
	position,
	rawVelocity,
	now,
	...
	if spawnDistance >= V47.C.RETURN_MIN_TRAVEL then
		local future =
			V47.predictReturnArcPoint(
				data,
				V47.C.RETURN_PREVIEW_TIME,
				position
			)
...
local function buildThreatSnapshot(root, humanoid)
		...
		then
			...
			for stepIndex = 1, TOTAL_PREDICTION_STEPS do
				...
				if p0 and p1 then
					...
					if data.MotionClass == "RETURN"
						and t <= V47.C.RETURN_SIDE_LOOKAHEAD
	...
	V47.State.TrajectoryStraight = straightCount
	V47.State.TrajectoryCurve = curveCount
	V47.State.TrajectoryWave = waveCount
	V47.State.TrajectoryReturn = returnCount

	return threats
end

function V47.getShieldCharge()
	...
	return tonumber(current) or 0, tonumber(maximum) or 2
end

function V47.getShieldCooldownRemaining()
	return math.max(
		0,
		V47.C.SHIELD_COOLDOWN
			- (os.clock() - V47.State.ShieldLastUse)
	)
end

function V47.findShieldPart()
	local core = workspace:FindFirstChild("Core__Game")
	if not core then
		return nil
	end
...
function V47.getActiveShieldModel()
	local now = os.clock()
	local shield = V47.findShieldPart()

	if shield then
		if V47.State.ShieldPart ~= shield then
			V47.State.ShieldPart = shield
			V47.State.ShieldCenter = shield.Position
			V47.State.ShieldUntil =
				now + V47.C.SHIELD_DURATION

			-- Also catches a manual shield use while the bot is running.
			if now - V47.State.ShieldLastUse > 1.0 then
				V47.State.ShieldLastUse = now
			end
		else
			V47.State.ShieldCenter = shield.Position
		end
		...
		local remaining =
			math.max(
				0,
				V47.State.ShieldUntil - now
			)

		if remaining > 0 then
			return {
				Center = V47.State.ShieldCenter,
				StartDelay = 0,
				Duration = remaining
			}
		end
	...
	if V47.State.ShieldPart ~= nil then
		V47.State.ShieldPart = nil
		V47.State.ShieldCenter = nil
		V47.State.ShieldUntil = 0
	end

	return nil
end

function V47.canUseShield()
	local charge =
		select(1, V47.getShieldCharge())

	if charge < V47.C.SHIELD_REQUIRED_CHARGE then
		V47.State.ShieldLastDecision = "NO CHARGE"
		return false
	end

	if V47.getShieldCooldownRemaining() > 0 then
		V47.State.ShieldLastDecision = "COOLDOWN"
		return false
	end

	if V47.getActiveShieldModel() then
		V47.State.ShieldLastDecision = "ACTIVE"
		return false
	end
	...
	if not event or not event:IsA("RemoteEvent") then
		V47.State.ShieldLastDecision = "NO REMOTE"
		return false
	end
...
function V47.useShield(root)
	if not root or not root.Parent then
		return false
	end

	if not V47.canUseShield() then
		return false
	end
	...
	V47.State.ShieldLastUse = now
	V47.State.ShieldCenter = root.Position
	V47.State.ShieldUntil =
		now
			+ V47.C.SHIELD_ACTIVATION_DELAY
			+ V47.C.SHIELD_DURATION

	V47.State.ShieldPlanHoldUntil =
		now + V47.C.SHIELD_PLAN_HOLD

	V47.State.ShieldUseCount += 1
	V47.State.ShieldLastDecision = "FIRED"

	event:FireServer(5, true)

	task.delay(
		V47.C.SHIELD_PRESS_TIME,
		function()
...
function V47.makeShieldThreatView(
	threats,
	center,
	startDelay,
	duration
...
local function constrainMoveAgainstBarriers(
	...
	then
		V47.State.WallConstraintState = "FREE"
	...
	V47.State.WallConstraintClearance =
	...
	V47.State.WallConstraintCount += 1

	if bestDirection then
		V47.State.WallConstraintState = "SLIDE"
		return bestDirection
	end

	V47.State.WallConstraintState = "STOP"
	return Vector3.zero
end
...
local function applyCurrentMove(
	...
	local requestedMove =
		V47.getExecutionDirection(root)
			or CurrentMove
	...
	for _, threat in ipairs(activeThreats) do
		...
		if motionClass == "RETURN" then
			...
			local standoffClearance =
				clearance - V47.C.RETURN_STANDOFF
			...
			if clearance < V47.C.RETURN_CORRIDOR_ZONE then
				local tangent =
					flat(
						newProjectile
							- oldProjectile
				...
				if tangent.Magnitude > 0.01 then
					...
					if preferredSide ~= 0 then
						...
						if shrink > 0 then
							risk +=
								shrink * shrink
									* V47.C.RETURN_SIDE_SHRINK_WEIGHT
									* (0.35 + closeness)
						end
					else
						if playerMove.Magnitude > 0.01 then
							...
							risk +=
								along * along
									* V47.C.RETURN_CENTERLINE_TANGENT_WEIGHT
									* (0.35 + closeness)
						end
					end
		...
		elseif motionClass == "WAVE"
			and clearance
				< V47.C.RETURN_GRAZE_BLOCK_ZONE * 0.70
		then
...
local function getNextInputs(previousDirection, depth, emergency)
	...
	local finePhase =
		not emergency
		and stepEnd <= V47.C.FINE_STEER_END
	...
	V47.State.FineSteerActive = finePhase
...
local function evaluateGrazeStep(
	...
	if unstableGrazeBlock
		...
		or clearance >= V47.C.GRAZE_MAX_CLEARANCE
	then
		return 0, 0
	end
	...
	if clearance <= V47.C.GRAZE_HARD_FLOOR then
		local danger =
			1 - math.clamp(
				clearance
					/ V47.C.GRAZE_HARD_FLOOR,
				0,
				1
			)

		return
			V47.C.GRAZE_TOO_CLOSE_PENALTY
				* (0.35 + danger * danger),
			0
	end

	local target =
		V47.C.GRAZE_TARGET_CLEARANCE

	local width =
		math.max(
			0.01,
			V47.C.GRAZE_MAX_CLEARANCE
				- target
		)
...
local function nodeSort(a, b)
	...
	if GRAZE_MODE
		and not V47.State.CornerActive
		and a.Hits == 0
		and b.Hits == 0
...
local function pruneBeamWithDiversity(
	...
	if depth < V47.C.DIVERSITY_START_DEPTH
		or keep < 4
	then
		return beam, 0
	end
...
local function planPath(root, humanoid, threats, cornerDirection)
	...
	V47.State.DiversityInjected = 0
	V47.State.NearMissBest = 0
	V47.State.FineSteerActive = false
	V47.State.GrazeScoreBest = 0

	local grazePlanning =
		GRAZE_MODE
			and not V47.State.CornerActive

	V47.State.GrazePlanning = grazePlanning
		...
		for _, node in ipairs(beam) do
			...
			local emergency =
				...
				or node.MinReturnClearance
					< V47.C.RETURN_EMERGENCY_CLEARANCE
			local inputs =
				getNextInputs(
					node.Dir,
					depth,
	...
	table.sort(finalists, function(a, b)
		...
		then
			...
			if (
				aReturn < V47.C.RETURN_CORRIDOR_ZONE
				or bReturn < V47.C.RETURN_CORRIDOR_ZONE
			)
			...
			if CurrentMove.Magnitude > 0.01
				...
				and math.abs(costDelta)
					<= V47.C.ROBUST_CURRENT_MOVE_EPSILON
			then
				local aDot =
	...
	V47.State.NearMissBest =
		best.Node.NearMisses or 0

	V47.State.GrazeScoreBest =
		best.Node.GrazeScore or 0

	V47.State.ReturnMinClearance =
		best.Node.MinReturnClearance
			or math.huge

	V47.State.ReturnStandoffActive =
		V47.State.ReturnMinClearance
			< V47.C.RETURN_STANDOFF
...
function V47.clearExecution()
	V47.State.ExecDirections = nil
	V47.State.ExecPositions = nil
	V47.State.ExecStartedAt = 0
	V47.State.ExecMaxAge = 0
	V47.State.ExecState = "FREE"
	V47.State.ExecStep = 0
	V47.State.ExecPositionError = 0
end

function V47.startExecution(plan)
	if not plan
	...
	local maxAge =
		V47.C.MPC_EXEC_NORMAL_AGE

	if plan.Hits > 0 then
		maxAge =
			V47.C.MPC_EXEC_RESCUE_AGE
	elseif GRAZE_MODE
		and V47.State.GrazePlanning
	then
		maxAge =
			V47.C.MPC_EXEC_GRAZE_AGE
	elseif V47.State.CornerActive then
		maxAge =
			V47.C.MPC_EXEC_CORNER_AGE
	end

	V47.State.ExecDirections = plan.Directions
	V47.State.ExecPositions = plan.Path
	V47.State.ExecStartedAt = os.clock()
	V47.State.ExecMaxAge = maxAge
	V47.State.ExecState = "ARMED"
	V47.State.ExecStep = 1
	V47.State.ExecCount += 1
	V47.State.ExecPositionError = 0
end

function V47.getExecutionStep(elapsed)
	for stepIndex = 1, PLAN_STEPS do
		if elapsed
			< getStepEndTime(stepIndex)
		then
...
function V47.getExecutionDirection(root)
	local directions =
		V47.State.ExecDirections
	...
	local elapsed =
		os.clock()
			- V47.State.ExecStartedAt

	if elapsed < 0
		or elapsed
			> V47.State.ExecMaxAge
	then
		V47.clearExecution()
		return nil
	end
	...
	if positions and positions[stepIndex] then
		...
		V47.State.ExecPositionError =
			errorDistance

		if errorDistance
			> V47.C.MPC_EXEC_POSITION_ERROR
		then
			V47.clearExecution()
			return nil
		end
	...
	if direction.Magnitude > 0.01 then
		local projected =
			root.Position
				+ direction.Unit
					* 16
					* V47.C.MPC_EXEC_WALL_CHECK_TIME
	...
	V47.State.ExecState = "FOLLOW"

	return direction
end

function V47.hasExecution()
	return
		V47.State.ExecDirections ~= nil
		and os.clock()
			- V47.State.ExecStartedAt
...
Main.Size = UDim2.fromOffset(260, 419)
...
Info.Size = UDim2.new(1, -20, 0, 166)
...
Toggle.Position = UDim2.fromOffset(10, 224)
...
GrazeToggle.Position = UDim2.fromOffset(10, 269)
...
RadiusLabel.Position = UDim2.fromOffset(42, 310)
...
Minus.Position = UDim2.fromOffset(10, 310)
...
Plus.Position = UDim2.new(1, -38, 0, 310)
...
PathToggle.Position = UDim2.fromOffset(10, 351)
...
Footer.Position = UDim2.fromOffset(10, 390)
Footer.BackgroundTransparency = 1
Footer.Text = "v47 / Full MPC Execution + Crisis Shield + Return"
...
Toggle.Activated:Connect(function()
	...
	if ENABLED then
		...
		V47.State.CornerActive = false
		V47.State.CornerDirection = nil
		V47.State.ShieldPlanHoldUntil = 0
		V47.State.ShieldLastDecision = "IDLE"
		V47.State.ShieldCrisisReason = "-"
		V47.State.WallConstraintState = "FREE"
		V47.State.ReturnMinClearance = math.huge
		V47.State.ReturnStandoffActive = false
		V47.clearExecution()
		clearPathVisual()
		enableControls()
...
local function decisionStep()
	...
	if now < V47.State.ShieldPlanHoldUntil then
	...
	local ok, err = pcall(function()
		...
		then
			...
			local imminentPlanHit =
				unavoidable
				and firstImpact
					<= V47.C.SHIELD_PANIC_IMPACT

			local urgentPlanHit =
				unavoidable
				and firstImpact
					<= V47.C.SHIELD_TRIGGER_IMPACT

			local fragilePlan =
				plan.Hits == 0
				and plan.MinClearance
					<= V47.C.SHIELD_FRAGILE_CLEARANCE
				and plan.EscapeOptions
					<= V47.C.SHIELD_FRAGILE_ESCAPE_OPTIONS

			local trappedPlan =
				V47.State.CornerActive
				and plan.EscapeOptions
					<= V47.C.SHIELD_FRAGILE_ESCAPE_OPTIONS
			...
			if crisis then
				if imminentPlanHit then
					V47.State.ShieldCrisisReason = "PANIC"
				elseif urgentPlanHit then
					V47.State.ShieldCrisisReason = "URGENT"
				elseif unavoidable then
					V47.State.ShieldCrisisReason = "UNAVOIDABLE"
				elseif trappedPlan then
					V47.State.ShieldCrisisReason = "TRAPPED"
				elseif fragilePlan then
					V47.State.ShieldCrisisReason = "FRAGILE"
				else
					V47.State.ShieldCrisisReason = "BASELINE"
				end
				...
				local materiallySafer =
					shieldMakesSafe
						and (
							unavoidable
							or clearanceGain
								>= V47.C.SHIELD_CLEARANCE_GAIN
							or escapeGain
								>= V47.C.SHIELD_ESCAPE_GAIN
						)
		...
		elseif activeShield then
			V47.State.ShieldCrisisReason = "ACTIVE"
			V47.State.ShieldLastDecision =
				"ACTIVE"
		else
		...
		if shieldUsed then
			if V47.State.ShieldLastDecision == "PANIC FIRE" then
				LastMode = "SHIELD PANIC"
			elseif V47.State.ShieldLastDecision == "RESCUE" then
		...
		Info.Text =
			...
			.. (
				V47.getActiveShieldModel()
					and "ACTIVE"
					or (
						V47.getShieldCooldownRemaining() > 0
							and (
								"CD "
								.. string.format(
									"%.0f",
									V47.getShieldCooldownRemaining()
								)
							)
							or V47.State.ShieldLastDecision
					)
			)
			.. " / "
			.. V47.State.ShieldCrisisReason
			.. " / Del "
			.. tostring(V47.State.ShieldLastRemoved)
			.. " / Sim "
			.. string.format(
				"%.1fms",
				V47.State.ShieldSimMs
			...
			.. "\nSearch D"
			.. tostring(V47.State.DiversityInjected)
			.. " / NM "
			.. tostring(V47.State.NearMissBest)
			.. " / Fine "
			.. (
				V47.State.FineSteerActive
					and "ON"
					or "-"
			)
			.. "\nGraze "
			...
			.. " / Fix "
			.. tostring(
				V47.State.WallConstraintCount
			)
			.. "\nTraj S"
			.. tostring(
				V47.State.TrajectoryStraight
			)
			.. " C"
			.. tostring(
				V47.State.TrajectoryCurve
			)
			.. " W"
			.. tostring(
				V47.State.TrajectoryWave
			)
			.. " R"
			.. tostring(
				V47.State.TrajectoryReturn
			)
			.. "\nRetPath "
			.. (
				V47.State.ReturnMinClearance
