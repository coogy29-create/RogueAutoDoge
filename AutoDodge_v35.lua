...
local V35 = {
	C = {
		...
		CAPSULE_SWEEP_SAMPLES = 3,
		SHAPE_EXACT_HORIZON = 1.15,
		EXACT_STEP_CAP_IMMEDIATE = 40,
		EXACT_STEP_CAP_NEAR = 30,
		EXACT_FORCE_CLEARANCE = 3.25,
		EXACT_FORCE_TIME = 0.72,
		WALL_GUARD_LOOKAHEAD = 0.14,
		WALL_GUARD_MIN_DISTANCE = 1.35,
		WALL_GUARD_TURN_ANGLES = {18, -18, 32, -32, 48, -48},
		PLAN_BACKOFF_SOFT_MS = 75,
		PLAN_BACKOFF_HARD_MS = 140,
		PLAN_BACKOFF_SOFT_INTERVAL = 0.060,
		PLAN_BACKOFF_HARD_INTERVAL = 0.095,

		URGENT_DODGE_TIME = 0.55,
		URGENT_LOCAL_HORIZON = 0.45,
		URGENT_CORNER_COST_SCALE = 0.70,
		URGENT_FINAL_TRAP_SCALE = 1.00,
		URGENT_TURN_TIE_WEIGHT = 7.0,
		URGENT_RISK_EPSILON = 0.80,
		URGENT_OPEN_REWARD = 20,
		URGENT_MIN_OPEN_REWARD = 34,
		URGENT_ESCAPE_REWARD = 90,
		URGENT_OPEN_EPSILON = 0.50,

		CORNER_LOCK_OPENNESS = 3.8,
		CORNER_LOCK_BARRIER_DISTANCE = 3.2,
		CORNER_DIRECTION_LOCK_TIME = 0.34,
		CORNER_SWITCH_PENALTY = 16,
		CORNER_SWITCH_DOT = 0.35,

		WALL_GUARD_SIDE_LOCK_TIME = 0.34,
		WALL_GUARD_REPLAN_COOLDOWN = 0.070,
	...
	State = {
		...
		WallGuard = false,
		WallGuardSide = 0,
		WallGuardSideUntil = 0,
		LastWallGuardReplan = 0,
		CornerLockDirection = nil,
		CornerLockUntil = 0,
		UrgentDirect = false
	}
}
...
function V35.getObjectBox(obj, part)
...
function V35.classifyGeometry(size)
	if not size then
		return "Circle"
	end
	...
	-- Narrow elongated bullets.
	if aspect >= V35.C.CAPSULE_MIN_ASPECT
		and shortAxis < V35.C.OBB_MIN_SHORT_AXIS
	then
		return "Capsule"
	end

	-- Broad elongated attacks.
	if aspect >= V35.C.CAPSULE_MIN_ASPECT then
		return "OBB"
	end

	-- Only genuinely large near-square footprints are OBB.
	if longAxis >= 5.0 or area >= 20.0 then
		return "OBB"
	end
...
function V35.getGeometryRadius(size)
	if not size then
		return 0.5
	end
...
function V35.getBroadRadius(size)
	if not size then
		return 0.5
	end
...
function V35.getLongAxis(cf, size)
...
function V35.segmentMinDistance2D(relativeA, relativeB)
	local a = flat(relativeA)
	local b = flat(relativeB)
	local d = b - a
	local denom = d:Dot(d)
	...
	return (a + d * t).Magnitude
end

function V35.pointSegmentDistance2D(point, a, b)
	local p = flat(point)
	local aa = flat(a)
	local bb = flat(b)
	local d = bb - aa
...
function V35.pointBoxSignedDistance2D(boxCf, boxSize, point, expansion)
...
function V35.getCapsuleSegment(cf, size, expansion)
	if not cf or not size then
		return nil, nil, 0
	end
	...
	local axis = V35.getLongAxis(cf, size)
...
function V35.capsuleSignedDistance2D(cf, size, point, expansion)
	local a, b, radius =
		V35.getCapsuleSegment(
			cf,
			size,
			expansion
		)
...
function V35.predictGeometryFrame(data, predictedPosition, t)
...
function V35.refreshGeometryTracking(data, obj, part, dt)
	local now = os.clock()
	...
	data.NextGeometryRefresh =
		now + V35.C.GEOMETRY_REFRESH

	local boxCf, boxSize =
		V35.getObjectBox(obj, part)
	...
	local previousAxis =
		data.ShapeAxis
			or V35.getLongAxis(boxCf, boxSize)

	local newAxis =
		V35.getLongAxis(boxCf, boxSize)
	...
	if dt and dt > 0 then
		...
		data.GeometryVelocity =
			(data.GeometryVelocity or rawVelocity):Lerp(
				rawVelocity,
				V35.C.GEOMETRY_VELOCITY_SMOOTH
			)
	...
	data.ShapeType =
		V35.classifyGeometry(boxSize)
	data.ShapeRadius =
		V35.getGeometryRadius(boxSize)
	data.BroadRadius =
		V35.getBroadRadius(boxSize)
	data.ShapeAxis = newAxis
	data.LastBoxPosition = boxCf.Position
end

function V35.shapeSignedDistance(
	shapeType,
	cf,
	size,
	point,
...
function V35.evaluateCapsuleSweep(
	oldCf,
	newCf,
	size,
	oldPlayer,
...
function V35.directionBarrierClearance(origin, direction, maxDistance)
...
function V35.positionIndexNearTime(targetTime)
	local bestIndex = 1
...
function V35.analyzeSlowFan(root, humanoid, threats)
	local sampleIndex =
		V35.positionIndexNearTime(
			V35.C.FAN_SAMPLE_TIME
		)
	...
	local occupied =
		table.create(V35.C.FAN_BINS, false)

	local binAngle =
		math.pi * 2 / V35.C.FAN_BINS
	...
	for _, threat in ipairs(threats) do
		...
		then
			...
			if future and current then
				...
				if distance >= 0.8
					and distance
						<= V35.C.FAN_MAX_DISTANCE
					and approach
						>= V35.C.FAN_MIN_APPROACH_DOT
				then
					count += 1
					centroid += future
					nearest =
	...
	if count < V35.C.FAN_MIN_PROJECTILES then
		if os.clock()
			<= V35.State.FanGraceUntil
			and V35.State.FanDirection
		then
			return {
				PreferredDirection =
					V35.State.FanDirection,
				Count = V35.State.FanCount,
				ArcDegrees = V35.State.FanArc,
				TimeToEdge =
					V35.State.FanEdgeTime,
				ClosingTime =
					V35.State.FanClosingTime
			}
		end

		V35.State.FanCount = 0
		V35.State.FanArc = 0
		V35.State.FanSide = "-"
		V35.State.FanDirection = nil
		return nil
	end
	...
	for i = 1, V35.C.FAN_BINS * 2 do
		local bin =
			((i - 1) % V35.C.FAN_BINS) + 1
	...
	local arcDegrees =
		longest * 360 / V35.C.FAN_BINS

	if arcDegrees < V35.C.FAN_MIN_ARC
		or arcDegrees > V35.C.FAN_MAX_ARC
	then
		return nil
	end
	...
	V35.State.FanCount = count
	V35.State.FanArc = arcDegrees
	V35.State.FanSide = side
	V35.State.FanDirection = preferred
	V35.State.FanGraceUntil =
		os.clock() + V35.C.FAN_GRACE
	V35.State.FanEdgeTime = timeToEdge
	V35.State.FanClosingTime = closingTime
...
function V35.fanCost(
	...
	local required =
		walkSpeed
			...
			* V35.C.FAN_PROGRESS_FRACTION

	if progress < required then
		cost +=
			(required - progress)
				* V35.C.FAN_PROGRESS_WEIGHT
	end
...
function V35.cornerStepCost(
	...
	then
		...
		cost +=
			approach
				* V35.C.CORNER_APPROACH_WEIGHT
				* nearFactor
	end

	if newClearance
		< V35.C.CORNER_CRITICAL_DISTANCE
	then
		local depth =
			V35.C.CORNER_CRITICAL_DISTANCE
				- newClearance
		...
		cost +=
			depth * depth
				* V35.C.CORNER_CRITICAL_WEIGHT
	end

	return cost
end

function V35.cornerFinalCost(
	rootAverage,
	rootMinimum,
	averageOpen,
	minimumOpen,
	...
	if averageOpen < rootAverage then
		cost +=
			(rootAverage - averageOpen)
				* V35.C.CORNER_OPEN_LOSS_WEIGHT
	end

	if minimumOpen < rootMinimum then
		cost +=
			(rootMinimum - minimumOpen)
				* V35.C.CORNER_MIN_OPEN_LOSS_WEIGHT
	end

	cost +=
		closeRays
			* V35.C.CORNER_CLOSE_RAY_EXTRA
...
RunService.Heartbeat:Connect(function(dt)
	...
	for obj, data in pairs(ProjectileData) do
		...
		if not part or not part.Parent then
			...
			data.Samples = 0
			V35.refreshGeometryTracking(data, obj, part, dt)
...
local function buildThreatSnapshot(root, humanoid)
			...
			do
				...
				then
					...
					local exactShape =
						(
							shapeType == "Circle"
							and t <= EXACT_THREAT_HORIZON
						)
						or (
							shapeType ~= "Circle"
							and t <= V35.C.SHAPE_EXACT_HORIZON
						)
	...
	for stepIndex = 1, TOTAL_PREDICTION_STEPS do
		...
		local rawCount = #bucket
		rawPeak = math.max(rawPeak, rawCount)

		local t = getStepEndTime(stepIndex)
		local nominalCap =
			t <= IMMEDIATE_THREAT_HORIZON
				and V35.C.EXACT_STEP_CAP_IMMEDIATE
				or V35.C.EXACT_STEP_CAP_NEAR

		if rawCount > nominalCap then
			local forced = {}
			local optional = {}

			for _, threat in ipairs(bucket) do
				local clearance =
					threat.StepPriority[stepIndex]
						or math.huge

				-- Never drop a projectile that is close enough to actually
				-- become a collision during the near-term plan.
				if clearance <= V35.C.EXACT_FORCE_CLEARANCE
					or (
						t <= V35.C.EXACT_FORCE_TIME
						and clearance <= NEAR_ZONE + 0.75
					)
				then
					forced[#forced + 1] = threat
				else
					optional[#optional + 1] = threat
				end
			end
			...
			table.sort(
				...
				function(a, b)
					return
						(a.StepPriority[stepIndex] or math.huge)
						< (b.StepPriority[stepIndex] or math.huge)
				end
			)
			...
			for _, threat in ipairs(forced) do
				rebuilt[#rebuilt + 1] = threat
			end

			local optionalBudget =
				math.max(
					0,
					nominalCap - #forced
				)

			for i = 1, math.min(optionalBudget, #optional) do
				rebuilt[#rebuilt + 1] = optional[i]
			end

			-- Only non-critical overflow becomes a far-field approximation.
			for i = optionalBudget + 1, #optional do
				local overflow = optional[i]
				local p =
					overflow.Positions
					and overflow.Positions[stepIndex + 1]

				if p then
					addFarFieldSample(
						farField[stepIndex],
						p,
						overflow.DangerRadius or 1
					)
				end
			end

			byStep[stepIndex] = rebuilt
			bucket = rebuilt
		end

		keptPeak = math.max(keptPeak, #bucket)
	end

	V35.State.ExactRawPeak = rawPeak
	V35.State.ExactKeptPeak = keptPeak
	...
	V35.State.CircleThreats = circleCount
	V35.State.CapsuleThreats = capsuleCount
	V35.State.OBBThreats = obbCount
...
local function planPath(root, humanoid, threats, fanInfo, urgentDodge)
	local beamWidth =
	...
	local nearCorner =
		rootMinimumOpen
			< V35.C.CORNER_LOCK_OPENNESS
		or rootBarrierClearance
			< V35.C.CORNER_LOCK_BARRIER_DISTANCE

	local continuityDirection = nil

	if nearCorner
		and V35.State.CornerLockDirection
		and os.clock() <= V35.State.CornerLockUntil
	then
		continuityDirection =
			V35.State.CornerLockDirection
	elseif nearCorner
		and CurrentMove.Magnitude > 0.01
	then
		continuityDirection = CurrentMove.Unit
	end
	...
	for depth = 1, PLAN_STEPS do
		...
		for _, node in ipairs(beam) do
			...
			for _, inputDirection in ipairs(inputs) do
				...
				if depth == 1
					and continuityDirection
					and direction.Magnitude > 0.01
				then
					local lockDot =
						math.clamp(
							continuityDirection:Dot(
								direction.Unit
							),
							-1,
							1
				...
				if urgentDodge
					and getStepEndTime(depth)
						<= V35.C.URGENT_LOCAL_HORIZON
				then
					cornerCost *=
						V35.C.URGENT_CORNER_COST_SCALE
				end
	...
	for i = 1, finalistCount do
		...
		if urgentDodge then
			trapCost *=
				V35.C.URGENT_FINAL_TRAP_SCALE
		end

		finalists[#finalists + 1] = {
			Node = node,
			FinalCost = node.Cost + trapCost,
			UrgentRisk = node.UrgentRisk,
			TurnAccum = node.TurnAccum,
			EscapeOptions = escapeOptions,
			EscapeTrend = escapeTrend,
			AverageOpen = averageOpen,
			MinimumOpen = minimumOpen,
			OpenScore =
				averageOpen * V35.C.URGENT_OPEN_REWARD
				+ minimumOpen * V35.C.URGENT_MIN_OPEN_REWARD
				+ escapeOptions * V35.C.URGENT_ESCAPE_REWARD
		}
	end
	...
	table.sort(finalists, function(a, b)
		...
		if urgentDodge then
			-- First: actually dodge the incoming projectile.
			local riskDelta =
				(a.UrgentRisk or 0)
					- (b.UrgentRisk or 0)

			if math.abs(riskDelta)
				> V35.C.URGENT_RISK_EPSILON
			then
				return riskDelta < 0
			end

			-- Second: among similarly safe dodges, move toward the route
			-- that ends in the widest / most escapable space.
			local openDelta =
				(a.OpenScore or 0)
					- (b.OpenScore or 0)

			if math.abs(openDelta)
				> V35.C.URGENT_OPEN_EPSILON
			then
				return openDelta > 0
			end

			-- Third: if safety and openness are effectively tied,
			-- prefer the less curved path.
			local aTurn =
				(a.TurnAccum or 0)
					* V35.C.URGENT_TURN_TIE_WEIGHT

			local bTurn =
				(b.TurnAccum or 0)
					* V35.C.URGENT_TURN_TIE_WEIGHT

			if math.abs(aTurn - bTurn) > 0.15 then
				return aTurn < bTurn
			end
		end
...
Footer.Text = "v35 / Dodge First + Open Space Bias + Corner Hysteresis"
...
Toggle.Activated:Connect(function()
	...
	if ENABLED then
		...
		CurrentPath = nil
		V35.State.CornerLockDirection = nil
		V35.State.CornerLockUntil = 0
		V35.State.WallGuardSide = 0
		V35.State.WallGuardSideUntil = 0
		V35.State.UrgentDirect = false
		clearPathVisual()
		enableControls()
...
local function wallGuardMove(root, humanoid, desiredMove)
	if desiredMove.Magnitude <= 0.01 then
		V35.State.WallGuard = false
		return Vector3.zero
	end

	local direction = desiredMove.Unit
	local probeDistance =
		math.max(
			V35.C.WALL_GUARD_MIN_DISTANCE,
			humanoid.WalkSpeed
				* V35.C.WALL_GUARD_LOOKAHEAD
		)

	local startPosition = root.Position
	local targetPosition =
		startPosition + direction * probeDistance

	if not pathHitsBarrier(startPosition, targetPosition) then
		V35.State.WallGuard = false

		if os.clock() > V35.State.WallGuardSideUntil then
			V35.State.WallGuardSide = 0
		end
	...
	local now = os.clock()
	local lockedSide =
		now <= V35.State.WallGuardSideUntil
			and V35.State.WallGuardSide
			or 0
	...
	local function searchSide(requiredSide)
		local foundDirection = nil
		local foundScore = -math.huge
		local foundSide = 0

		for _, degrees in ipairs(
			V35.C.WALL_GUARD_TURN_ANGLES
		) do
			local side =
				degrees > 0 and 1 or -1

			if requiredSide == 0
				or side == requiredSide
			then
				local candidate =
					rotateY(
						direction,
				...
				if not pathHitsBarrier(
					...
					if score > foundScore then
						foundScore = score
						foundDirection = candidate
						foundSide = side
					end
				end
			end
	...
	if lockedSide ~= 0 then
		bestDirection,
			bestScore,
			bestSide =
			searchSide(lockedSide)
	end

	-- Locked side is physically blocked: only then permit a side swap.
	...
	V35.State.WallGuard = true

	if bestDirection then
		if bestSide ~= 0 then
			V35.State.WallGuardSide = bestSide
			V35.State.WallGuardSideUntil =
				now
					+ V35.C.WALL_GUARD_SIDE_LOCK_TIME
		end

		if now - V35.State.LastWallGuardReplan
			>= V35.C.WALL_GUARD_REPLAN_COOLDOWN
		then
			LastPlanTime = 0
			V35.State.LastWallGuardReplan = now
		end

		return bestDirection
	end

	-- Completely boxed by barriers: don't oscillate; stop for this frame.
	if now - V35.State.LastWallGuardReplan
		>= V35.C.WALL_GUARD_REPLAN_COOLDOWN
	then
		LastPlanTime = 0
...
local function getPlanInterval()
	...
	local base =
		LastThreatCount > 0
			and PLAN_INTERVAL_ACTIVE
			or PLAN_INTERVAL_IDLE

	-- Backoff only when there is no near-term impact.
	if LastPlanMs >= V35.C.PLAN_BACKOFF_HARD_MS then
		return math.max(
			base,
			V35.C.PLAN_BACKOFF_HARD_INTERVAL
		)
	end

	if LastPlanMs >= V35.C.PLAN_BACKOFF_SOFT_MS then
		return math.max(
			base,
			V35.C.PLAN_BACKOFF_SOFT_INTERVAL
		)
	end
...
local function decisionStep()
	...
	if now - LastPlanTime < getPlanInterval() then
		local guarded =
			wallGuardMove(root, humanoid, CurrentMove)
		humanoid:Move(guarded, false)
		return
	end
	...
	local ok, err = pcall(function()
		...
		if #threats == 0 then
			V35.State.CircleThreats = 0
			V35.State.CapsuleThreats = 0
			V35.State.OBBThreats = 0
			V35.State.FanCount = 0
			V35.State.FanArc = 0
			V35.State.FanSide = "-"
			V35.State.FanDirection = nil
		...
		local urgentDodge =
			...
			and baseline.EarliestImpact
				<= V35.C.URGENT_DODGE_TIME

		V35.State.UrgentDirect = urgentDodge
		...
		local _, rootMinimumOpenNow =
			barrierOpenness(root.Position)

		local rootBarrierNow =
			nearestBarrierDistance(root.Position)
		...
		if nearCornerNow
			and CurrentMove.Magnitude > 0.01
		then
			if not V35.State.CornerLockDirection
				or os.clock()
					> V35.State.CornerLockUntil
				or V35.State.CornerLockDirection:Dot(
					CurrentMove.Unit
				) > 0.35
			then
				V35.State.CornerLockDirection =
				...
				V35.State.CornerLockUntil =
					os.clock()
						+ V35.C.CORNER_DIRECTION_LOCK_TIME
			end
		elseif not nearCornerNow then
			V35.State.CornerLockDirection = nil
			V35.State.CornerLockUntil = 0
		end
...
task.spawn(function()
	while Gui.Parent do
		...
		Info.Text =
			...
			.. "\nGeo C"
			.. tostring(V35.State.CircleThreats)
			.. " K"
			.. tostring(V35.State.CapsuleThreats)
			.. " B"
			.. tostring(V35.State.OBBThreats)
			.. "\nX "
			.. tostring(V35.State.ExactRawPeak)
			.. ">"
			.. tostring(V35.State.ExactKeptPeak)
			.. " / W "
			.. (
				V35.State.WallGuard
					and "GUARD"
			...
			.. " / WS "
			.. tostring(V35.State.WallGuardSide)
			.. "\nFan "
			.. tostring(V35.State.FanCount)
			.. " / "
			.. string.format(
				"%.0fdeg",
				V35.State.FanArc
