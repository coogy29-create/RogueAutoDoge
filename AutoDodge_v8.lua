...
local FAN_DIRECTION_WEIGHT = 46
local FAN_REVERSE_EXTRA = 75
local FAN_STOP_PENALTY = 130
...
local PREPROCESS_SLICE_BUDGET = 0.0025
local PREPROCESS_YIELD_EVERY = 24
...
local function buildThreatSnapshot(root, humanoid, yieldCallback)
	...
	for _, data in pairs(ProjectileData) do
		...
		if yieldCallback
			and processedProjectiles % PREPROCESS_YIELD_EVERY == 0
		then
			yieldCallback()
		end
...
local function analyzeDenseFront(root, humanoid, threats, yieldCallback)
	...
	for _, threat in ipairs(threats) do
		scannedThreats += 1
		if yieldCallback
			and scannedThreats % PREPROCESS_YIELD_EVERY == 0
		then
			yieldCallback()
		end
...
local function immediateProjectilePosition(data, t)
	...
	if direction.Magnitude > 0.01 then
		local turn = data.TurnRate or 0
		if math.abs(turn) > 0.001 then
			-- Short-horizon curved-motion approximation. The full planner still
			-- uses the more expensive 1.5-second predictor after movement starts.
			direction = rotateY(direction, turn * t * 0.5)
		end
	end
	...
	local travel =
		math.max(
			0,
			speed * t
				+ 0.5 * (data.SpeedAccel or 0) * t * t
		)

	local y =
		origin.Y
			+ velocity.Y * t
			+ 0.5 * (data.VerticalAccel or 0) * t * t
...
local function getImmediateMPCInputs(previousDirection, depth, emergency)
	if depth == 1 then
...
local function planImmediateMPC(root, humanoid, threats)
	local dense =
		#threats >= math.min(
			DENSE_THREAT_COUNT,
	...
	for depth = 1, IMMEDIATE_MPC_STEPS do
		...
		for _, node in ipairs(beam) do
			...
			for _, inputDirection in ipairs(inputs) do
				...
				-- Reuse the most recent dense-front direction as a prior. The
				-- immediate layer remains MPC; this only biases its candidate cost.
				local fanCost = 0
				if LastFanDirection.Magnitude > 0.01
					and LastFanCount >= FAN_MIN_PROJECTILES
					and direction.Magnitude > 0.01
				then
					local dot =
						math.clamp(
							direction.Unit:Dot(
								LastFanDirection.Unit
							),
							-1,
							1
						)
...
local function dominanceKey(node)
	...
	return tostring(ix) .. ":" .. tostring(iz) .. ":" .. tostring(dirBin)
end
...
local function planPath(root, humanoid, threats, denseFront)
	...
	local function maybeYield()
		expansionCounter += 1
		if expansionCounter % PLANNER_YIELD_EVERY == 0
			and os.clock() - sliceStarted >= PLANNER_SLICE_BUDGET
		then
	...
	for depth = 1, PLAN_STEPS do
		...
		for _, node in ipairs(beam) do
			...
			for _, inputDirection in ipairs(inputs) do
				...
				then
					if direction.Magnitude <= 0.01 then
						fanCost += FAN_STOP_PENALTY
					else
						local dot =
							math.clamp(
								direction.Unit:Dot(
									denseFront.PreferredDirection
								),
								-1,
								1
							)
						fanCost +=
							(1 - dot) * FAN_DIRECTION_WEIGHT
						if dot < -0.10 then
							fanCost += (-dot) * FAN_REVERSE_EXTRA
						end
					end
				end
...
Footer.Text = "0.55s MPC NOW / 1.5s MPC / FAN Escape"
...
local function decisionStep()
	...
	local ok, err = pcall(function()
		...
		local denseFront = analyzeDenseFront(root, humanoid, threats, preprocessYield)
		if denseFront then
			LastFanCount = denseFront.Count or 0
			LastFanArc = denseFront.ArcDegrees or 0
			LastFanDirection = denseFront.PreferredDirection or Vector3.zero
		else
			LastFanCount = 0
			LastFanArc = 0
			LastFanDirection = Vector3.zero
		...
		local plan = planPath(root, humanoid, threats, denseFront)

		if not plan then
			-- Keep the last valid movement instead of freezing while a new plan fails.
			LastEscapeOptions = 0
			LastEscapeTrendText = "-"
			LastMode = denseFront and "FAN HOLD" or "NO PLAN"
			return
		end
