...
warn("[AutoDodge v21] boot / core helpers restored")
...
local function runImmediateMPC()
	...
	-- FAN geometry changes much more slowly than collision response. Running
	-- five future FAN projections every 18 ms was starving the actual dodge
	-- MPC. Refresh the FAN model at its own cadence and reuse it between scans.
	...
	local dense = #threats >= C.DENSE_THREAT_COUNT
	local urgent =
		fanInfo ~= nil
		or baseline.Hits > 0
		or baseline.MinClearance
			<= (
				dense
				and C.IMMEDIATE_MPC_PROACTIVE_DENSE
				or C.IMMEDIATE_MPC_TRIGGER_CLEARANCE
			)
...
Footer.Text = "v21 / Core Helpers Restored / Fast MPC"
