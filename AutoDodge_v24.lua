...
warn("[AutoDodge v24] boot / single immediate MPC")
...
local function getNextInputs(previousDirection, depth, emergency, denseFront)
	...
	if previousDirection.Magnitude < 0.01 then
		...
		local farPhase = getStepEndTime(depth) > C.EXACT_THREAT_HORIZON
	...
	if denseFront
		...
		and getStepEndTime(depth) <= C.FAN_ESCAPE_COMMIT_TIME
	then
...
local function planPath(root, humanoid, threats, denseFront)
	local beamWidth =
		getAdaptiveBeamWidth(
			S.LastImmediateThreatCount + S.LastNearThreatCount
		)
	...
	local function maybeYield()
		...
		if expansionCounter % C.PLANNER_CLOCK_CHECK_EVERY == 0 then
	...
	for depth = 1, C.PLAN_STEPS do
		...
		for _, node in ipairs(beam) do
			...
			local allowFocus =
				denseFront ~= nil
				or emergency
				or node.MinClearance <= C.FOCUS_CONSIDER_CLEARANCE
				or (
					S.LastImmediateThreatCount
						+ S.LastNearThreatCount
				) >= C.FOCUS_CONSIDER_THREATS

			for _, inputDirection in ipairs(inputs) do
				local direction = clampUnit(inputDirection)
				local focusOptions
				if depth <= IMMEDIATE_MPC_STEPS then
					focusOptions =
						getFocusOptions(
							node,
							allowFocus,
				...
				elseif node.Dir.Magnitude > 0.01
					and direction.Magnitude <= 0.01
				then
					turnCost = C.TURN_COST_WEIGHT * 0.28
				end

				local continuityMove = CurrentMove
				...
				then
					...
					turnCost +=
						(1 - dot)
						* C.TURN_COST_WEIGHT
						* C.FULL_CONTINUITY_WEIGHT

					if dot < 0 then
						turnCost +=
							(-dot)
							* C.FULL_REVERSE_PENALTY
					end
				end
				...
				local moveCost =
					moveDistance * C.MOVE_COST_WEIGHT
					+ speedChange * C.SPEED_CHANGE_COST_WEIGHT
					+ turnCost
		...
		if os.clock() - sliceStarted >= getPlannerSliceBudget() then
			RunService.Heartbeat:Wait()
			sliceStarted = os.clock()
		end
	end
	...
	local dense =
		(S.LastImmediateThreatCount + S.LastNearThreatCount)
			>= C.DENSE_THREAT_COUNT
	local finalistLimit =
		dense and C.DENSE_FINALIST_COUNT or C.FINALIST_COUNT
	...
	for i = 1, finalistCount do
		...
		if escapeOptions == 0 then
			...
			trapCost -= escapeOptions * C.ESCAPE_OPTION_REWARD
		end

		trapCost +=
			math.max(0, 4.0 - minimumOpen) * 18
			+ closeRays * C.BARRIER_CLOSE_RAY_PENALTY
...
Footer.Text = "v24 / Single Immediate MPC / No Emergency"
...
local function decisionStep()
	...
	local now = os.clock()
	if not S.ForceFullReplan
		and now - S.LastPlanTime < getPlanInterval()
	then
		return
	end
	...
	local started = os.clock()
	local planStartPosition = root.Position
	...
	local ok, err = pcall(function()
		...
		S.LastMinClearance = baseline.MinClearance
		S.LastImpactTime = baseline.EarliestImpact
		S.LastFarRisk = baseline.FarRisk or 0
		...
		local activationLookahead =
			exactThreatCount >= C.DENSE_THREAT_COUNT
		...
		local needPlan =
			...
			or baseline.MinClearance <= C.PROACTIVE_CLEARANCE
			or (
				exactThreatCount >= C.DENSE_THREAT_COUNT
				and baseline.MinClearance <= C.DENSE_PROACTIVE_CLEARANCE
			)
			or (baseline.FarRisk or 0) >= C.FAR_ACTIVATION_RISK

		if not needPlan then
			local keepRecentMove =
				CurrentMove.Magnitude > 0.01
				and os.clock() - S.LastFullPlanCommitAt
					<= C.FULL_MOVE_RELEASE_GRACE

			if not keepRecentMove
				and not ImmediateMPCActive
			then
				CurrentMove = Vector3.zero
				CurrentPlanFocus = false
				CurrentPath = nil
		...
		local planAge = os.clock() - started
		local planTravel =
			flat(root.Position - planStartPosition).Magnitude
		local stalePlan =
			planAge >= C.FULL_PLAN_STALE_HARD_AGE
			or (
				planAge >= C.FULL_PLAN_STALE_SOFT_AGE
				and planTravel
					>= C.FULL_PLAN_STALE_DISTANCE
			)

		if stalePlan then
			S.StalePlanDiscards += 1
			S.ForceFullReplan = true
			S.LastMode = "STALE"
			return
		end

		-- Full-plan buffer: this is the normal actuator path. Emergency is
		-- allowed to override only while this move is actually collision-bound.
		CurrentMove = plan.Direction
		CurrentPlanFocus = false
		CurrentPath = plan.Path
		S.FullPlanGeneration += 1
		S.LastFullPlanCommitAt = os.clock()
		S.LastMinClearance = plan.MinClearance
		S.LastEscapeOptions = plan.EscapeOptions
		if plan.EscapeTrend then
			local trendText = {}
			for i, value in ipairs(plan.EscapeTrend) do
				trendText[i] = tostring(value)
			end
			S.LastEscapeTrendText = table.concat(trendText, ">")
		else
			S.LastEscapeTrendText = "-"
		end
		...
		elseif baseline.Hits == 0
			and (baseline.FarRisk or 0) >= C.FAR_ACTIVATION_RISK
		then
			S.LastMode = "PREP"
		elseif exactThreatCount >= C.DENSE_THREAT_COUNT then
			S.LastMode = "DENSE"
		else
			S.LastMode = "DODGE"
		end
	...
	S.LastPlanMs = (os.clock() - started) * 1000
...
-- v24: no background Full MPC loop.
-- The fast receding-horizon MPC is the single source of movement decisions.

-- RenderStep only applies the latest first action from the single fast MPC.
...
task.spawn(function()
	while Gui.Parent do
		...
		Info.Text =
			...
			.. "\nImpact "
			.. formatImpact(S.LastImpactTime)
			.. " / Clear "
			.. formatClearance(S.LastMinClearance)
			.. " / Far "
			.. string.format("%.1f", S.LastFarRisk)
			.. "\nMPC0.55 "
			.. (ImmediateMPCActive and "ON" or "OFF")
			.. " / T "
			.. tostring(S.LastImmediateMPCThreats)
			.. " / MC "
			.. formatClearance(S.LastImmediateMPCClearance)
			.. " / "
			.. string.format("%.1fms", S.LastImmediateMPCMs)
			.. " / A "
			.. tostring(S.LastImmediateMPCActionCount)
			.. "\nEscape "
			.. tostring(S.LastEscapeOptions)
			.. " ["
			.. S.LastEscapeTrendText
			.. "] / "
			.. string.format("%.1fms", S.LastPlanMs)
			.. "\nFan "
			.. (getFanInfo(os.clock()) and "MPC" or "OFF")
			.. " / "
			.. tostring(S.LastFanCount)
			.. " / "
			.. string.format("%.0f°", S.LastFanArc)
			.. " / +"
			.. string.format("%.2fs", S.LastFanForecastTime)
			.. "\nGrid "
			.. tostring(S.LastSpatialQueryCount)
			.. " / LB "
			.. tostring(#LargeBarrierParts)
			.. " / Slice "
			.. string.format("%.2fms", S.LastPlannerSliceBudgetMs)
			.. " / FPS~"
			.. string.format("%.0f", 1 / math.max(S.FrameDtEMA, 0.001))
			.. (
				S.LastError ~= ""
				and ("\n" .. string.sub(S.LastError, 1, 42))
				or ""
			)
