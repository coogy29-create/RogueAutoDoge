...
function V36.directionOpenDistance(position, direction)
	local nearest = V36.C.CORNER_PROBE_DISTANCE
...
function V36.bestCornerExit(position)
	local bestDirection = nil
	local bestScore = -math.huge

	for i = 0, V36.C.CORNER_PROBE_DIRECTIONS - 1 do
		local angle =
			i * math.pi * 2
				/ V36.C.CORNER_PROBE_DIRECTIONS
...
function V36.updateCornerObjective(position)
	local _, minimumOpen, closeRays =
		barrierOpenness(position)

	-- Once we have actually left the corner, release the objective.
	if minimumOpen >= V36.C.CORNER_RELEASE_OPEN
		and closeRays < V36.C.CORNER_CLOSE_RAYS
	then
		V36.State.CornerActive = false
		V36.State.CornerDirection = nil
		return nil
	end

	local trapped =
		minimumOpen < V36.C.CORNER_MIN_OPEN
		and closeRays >= V36.C.CORNER_CLOSE_RAYS

	-- Critical anti-wiggle behavior:
	-- keep the SAME exit direction as long as that channel still exists.
	if V36.State.CornerActive
		and V36.State.CornerDirection
	then
		local clearance =
			V36.directionOpenDistance(
				position,
				V36.State.CornerDirection
			)

		if clearance
			>= V36.C.CORNER_EXIT_MIN_CLEARANCE
		then
			return V36.State.CornerDirection
		end
	...
	local direction =
		V36.bestCornerExit(position)

	if direction then
		V36.State.CornerActive = true
		V36.State.CornerDirection = direction
	end

	return direction
end

function V36.cornerRouteCost(
	oldPosition,
	newPosition,
	direction,
	cornerDirection
...
Footer.Text = "v36 / Clean V4 + Persistent Corner Exit"
...
local function decisionStep()
	...
	local ok, err = pcall(function()
		...
		if plan.Hits > 0 then
			LastMode = "RESCUE"
		elseif V36.State.CornerActive then
