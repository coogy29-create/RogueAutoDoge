...
warn("[AutoDodge v22] boot / Full-MPC ownership + emergency filter")
...
C.IMMEDIATE_MPC_TURN_WEIGHT = 1.05
...
local function evaluateCurrentMoveSafety(root, humanoid, threats)
	local position = root.Position
	local velocity = flat(root.AssemblyLinearVelocity)
	local direction = clampUnit(CurrentMove)
...
local function runImmediateMPC()
	...
	-- FAN belongs to the full MPC cost model. The emergency layer never turns
	-- on merely because FAN information exists.
	...
	-- Safety-filter the movement that the 1.5s MPC is ACTUALLY asking us to do.
	local currentSafety =
		evaluateCurrentMoveSafety(
			root,
			humanoid,
			threats
		)
	...
	S.LastImmediateMPCClearance =
		currentSafety.MinClearance
	S.LastEmergencyImpact =
		currentSafety.EarliestImpact

	local unsafe =
		currentSafety.Hits > 0
		or currentSafety.WallHits > 0
		or currentSafety.MinClearance
			<= C.EMERGENCY_TRIGGER_CLEARANCE
	...
	S.LastCurrentPlanSafety = "EMERGENCY"
...
Footer.Text = "v22 / Full MPC Owner / 0.30s Emergency Filter"
...
local function decisionStep()
	...
	local now = os.clock()
	if now - S.LastPlanTime < getPlanInterval() then
		return
	end
	...
	local ok, err = pcall(function()
		...
		-- Full-plan buffer: this is the normal actuator path. The short planner
		-- may override only when this exact move fails the 0.30s safety check.
...
-- RenderStep performs no search. The 1.5s Full MPC owns movement normally;
-- the short MPC is only an emergency safety override when that exact move is
-- predicted to collide within the near horizon.
...
task.spawn(function()
	while Gui.Parent do
		...
		Info.Text =
			...
			.. " / EI "
			.. formatImpactTime(S.LastEmergencyImpact)
			.. " / "
