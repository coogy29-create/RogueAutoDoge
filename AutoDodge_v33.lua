...
function V33.getObjectBox(obj, part)
...
function V33.classifyGeometry(size)
	if not size then
		return "Circle"
	end
	...
	if aspect >= V33.C.CAPSULE_MIN_ASPECT
		and shortAxis < V33.C.OBB_MIN_SHORT_AXIS
	then
		return "Capsule"
	end

	-- Broad elongated attacks.
	if aspect >= V33.C.CAPSULE_MIN_ASPECT then
		return "OBB"
	end
...
function V33.getGeometryRadius(size)
	if not size then
		return 0.5
	end
...
function V33.getBroadRadius(size)
	if not size then
		return 0.5
	end
...
function V33.getLongAxis(cf, size)
...
function V33.segmentMinDistance2D(relativeA, relativeB)
...
function V33.pointSegmentDistance2D(point, a, b)
	local p = flat(point)
	local aa = flat(a)
	local bb = flat(b)
	local d = bb - aa
...
function V33.pointBoxSignedDistance2D(boxCf, boxSize, point, expansion)
...
function V33.getCapsuleSegment(cf, size, expansion)
	if not cf or not size then
		return nil, nil, 0
	end
	...
	local axis = V33.getLongAxis(cf, size)
...
function V33.capsuleSignedDistance2D(cf, size, point, expansion)
	local a, b, radius =
		V33.getCapsuleSegment(
			cf,
			size,
			expansion
		)
...
function V33.predictGeometryFrame(data, predictedPosition, t)
...
function V33.refreshGeometryTracking(data, obj, part, dt)
	local now = os.clock()
	...
	data.NextGeometryRefresh =
		now + V33.C.GEOMETRY_REFRESH

	local boxCf, boxSize =
		V33.getObjectBox(obj, part)
	...
	local previousAxis =
		data.ShapeAxis
			or V33.getLongAxis(boxCf, boxSize)

	local newAxis =
		V33.getLongAxis(boxCf, boxSize)
	...
	if dt and dt > 0 then
		...
		data.GeometryVelocity =
			(data.GeometryVelocity or rawVelocity):Lerp(
				rawVelocity,
				V33.C.GEOMETRY_VELOCITY_SMOOTH
			)
	...
	data.ShapeType =
		V33.classifyGeometry(boxSize)
	data.ShapeRadius =
		V33.getGeometryRadius(boxSize)
	data.BroadRadius =
		V33.getBroadRadius(boxSize)
	data.ShapeAxis = newAxis
	data.LastBoxPosition = boxCf.Position
end

function V33.shapeSignedDistance(
	shapeType,
	cf,
	size,
	point,
...
function V33.evaluateCapsuleSweep(
	oldCf,
	newCf,
	size,
	oldPlayer,
...
function V33.directionBarrierClearance(origin, direction, maxDistance)
...
function V33.positionIndexNearTime(targetTime)
	local bestIndex = 1
...
function V33.analyzeSlowFan(root, humanoid, threats)
	local sampleIndex =
		V33.positionIndexNearTime(
			V33.C.FAN_SAMPLE_TIME
		)
	...
	local occupied =
		table.create(V33.C.FAN_BINS, false)

	local binAngle =
		math.pi * 2 / V33.C.FAN_BINS
	...
	for _, threat in ipairs(threats) do
		...
		then
			...
			if future and current then
				...
				if distance >= 0.8
					and distance
						<= V33.C.FAN_MAX_DISTANCE
					and approach
						>= V33.C.FAN_MIN_APPROACH_DOT
				then
					count += 1
					centroid += future
					nearest =
	...
	if count < V33.C.FAN_MIN_PROJECTILES then
		if os.clock()
			<= V33.State.FanGraceUntil
			and V33.State.FanDirection
		then
			return {
				PreferredDirection =
					V33.State.FanDirection,
				Count = V33.State.FanCount,
				ArcDegrees = V33.State.FanArc,
				TimeToEdge =
					V33.State.FanEdgeTime,
				ClosingTime =
					V33.State.FanClosingTime
			}
		end

		V33.State.FanCount = 0
		V33.State.FanArc = 0
		V33.State.FanSide = "-"
		V33.State.FanDirection = nil
		return nil
	end
	...
	for i = 1, V33.C.FAN_BINS * 2 do
		local bin =
			((i - 1) % V33.C.FAN_BINS) + 1
	...
	local arcDegrees =
		longest * 360 / V33.C.FAN_BINS

	if arcDegrees < V33.C.FAN_MIN_ARC
		or arcDegrees > V33.C.FAN_MAX_ARC
	then
		return nil
	end
	...
	V33.State.FanCount = count
	V33.State.FanArc = arcDegrees
	V33.State.FanSide = side
	V33.State.FanDirection = preferred
	V33.State.FanGraceUntil =
		os.clock() + V33.C.FAN_GRACE
	V33.State.FanEdgeTime = timeToEdge
	V33.State.FanClosingTime = closingTime
...
function V33.fanCost(
	...
	local required =
		walkSpeed
			...
			* V33.C.FAN_PROGRESS_FRACTION

	if progress < required then
		cost +=
			(required - progress)
				* V33.C.FAN_PROGRESS_WEIGHT
	end
...
function V33.cornerStepCost(
	...
	then
		...
		cost +=
			approach
				* V33.C.CORNER_APPROACH_WEIGHT
				* nearFactor
	end

	if newClearance
		< V33.C.CORNER_CRITICAL_DISTANCE
	then
		local depth =
			V33.C.CORNER_CRITICAL_DISTANCE
				- newClearance
		...
		cost +=
			depth * depth
				* V33.C.CORNER_CRITICAL_WEIGHT
	end

	return cost
end

function V33.cornerFinalCost(
	rootAverage,
	rootMinimum,
	averageOpen,
	minimumOpen,
	...
	if averageOpen < rootAverage then
		cost +=
			(rootAverage - averageOpen)
				* V33.C.CORNER_OPEN_LOSS_WEIGHT
	end

	if minimumOpen < rootMinimum then
		cost +=
			(rootMinimum - minimumOpen)
				* V33.C.CORNER_MIN_OPEN_LOSS_WEIGHT
	end

	cost +=
		closeRays
			* V33.C.CORNER_CLOSE_RAY_EXTRA
...
RunService.Heartbeat:Connect(function(dt)
	...
	for obj, data in pairs(ProjectileData) do
		...
		if not part or not part.Parent then
			...
			data.Samples = 0
			V33.refreshGeometryTracking(data, obj, part, dt)
...
local function buildThreatSnapshot(root, humanoid)
			...
			do
				...
				then
					...
					local exactShape =
						...
						or (
							shapeType ~= "Circle"
							and t <= V33.C.SHAPE_EXACT_HORIZON
						)
	...
	for stepIndex = 1, TOTAL_PREDICTION_STEPS do
		...
		local nominalCap =
			t <= IMMEDIATE_THREAT_HORIZON
				and V33.C.EXACT_STEP_CAP_IMMEDIATE
				or V33.C.EXACT_STEP_CAP_NEAR
		...
		if rawCount > nominalCap then
			...
			for _, threat in ipairs(bucket) do
				...
				if clearance <= V33.C.EXACT_FORCE_CLEARANCE
					or (
						t <= V33.C.EXACT_FORCE_TIME
	...
	V33.State.ExactRawPeak = rawPeak
	V33.State.ExactKeptPeak = keptPeak
	...
	V33.State.CircleThreats = circleCount
	V33.State.CapsuleThreats = capsuleCount
	V33.State.OBBThreats = obbCount
...
Footer.Text = "v33 / V4 Brain / Critical Exact + Wall Guard + Fan"
...
local function wallGuardMove(root, humanoid, desiredMove)
	...
	local probeDistance =
		math.max(
			V33.C.WALL_GUARD_MIN_DISTANCE,
			humanoid.WalkSpeed
				* V33.C.WALL_GUARD_LOOKAHEAD
		)
	...
	for _, degrees in ipairs(V33.C.WALL_GUARD_TURN_ANGLES) do
		local candidate =
			rotateY(direction, math.rad(degrees))

		local candidateTarget =
			startPosition
				+ candidate * probeDistance

		if not pathHitsBarrier(startPosition, candidateTarget) then
			local alignment = candidate:Dot(direction)
	...
	V33.State.WallGuard = true

	-- A wall-clipped command is stale. Let the full MPC replace it immediately.
	LastPlanTime = 0
...
local function getPlanInterval()
	...
	if LastPlanMs >= V33.C.PLAN_BACKOFF_HARD_MS then
		return math.max(
			base,
			V33.C.PLAN_BACKOFF_HARD_INTERVAL
		)
	end

	if LastPlanMs >= V33.C.PLAN_BACKOFF_SOFT_MS then
		return math.max(
			base,
			V33.C.PLAN_BACKOFF_SOFT_INTERVAL
		)
	end
...
local function decisionStep()
	...
	local ok, err = pcall(function()
		...
		if #threats == 0 then
			V33.State.CircleThreats = 0
			V33.State.CapsuleThreats = 0
			V33.State.OBBThreats = 0
			V33.State.FanCount = 0
			V33.State.FanArc = 0
			V33.State.FanSide = "-"
			V33.State.FanDirection = nil
...
task.spawn(function()
	while Gui.Parent do
		...
		Info.Text =
			...
			.. "\nGeo C"
			.. tostring(V33.State.CircleThreats)
			.. " K"
			.. tostring(V33.State.CapsuleThreats)
			.. " B"
			.. tostring(V33.State.OBBThreats)
			.. "\nX "
			.. tostring(V33.State.ExactRawPeak)
			.. ">"
			.. tostring(V33.State.ExactKeptPeak)
			.. " / W "
			.. (
				V33.State.WallGuard
					and "GUARD"
