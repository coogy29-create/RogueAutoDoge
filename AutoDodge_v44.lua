...
function V44.directionOpenDistance(position, direction)
	local nearest = V44.C.CORNER_PROBE_DISTANCE
...
function V44.bestCornerExit(position)
	local bestDirection = nil
	local bestScore = -math.huge

	for i = 0, V44.C.CORNER_PROBE_DIRECTIONS - 1 do
		local angle =
			i * math.pi * 2
				/ V44.C.CORNER_PROBE_DIRECTIONS
...
function V44.updateCornerObjective(position)
	local _, minimumOpen, closeRays =
		barrierOpenness(position)

	-- Once we have actually left the corner, release the objective.
	if minimumOpen >= V44.C.CORNER_RELEASE_OPEN
		and closeRays < V44.C.CORNER_CLOSE_RAYS
	then
		V44.State.CornerActive = false
		V44.State.CornerDirection = nil
		return nil
	end

	local trapped =
		minimumOpen < V44.C.CORNER_MIN_OPEN
		and closeRays >= V44.C.CORNER_CLOSE_RAYS

	-- Critical anti-wiggle behavior:
	-- keep the SAME exit direction as long as that channel still exists.
	if V44.State.CornerActive
		and V44.State.CornerDirection
	then
		local clearance =
			V44.directionOpenDistance(
				position,
				V44.State.CornerDirection
			)

		if clearance
			>= V44.C.CORNER_EXIT_MIN_CLEARANCE
		then
			return V44.State.CornerDirection
		end
	...
	local direction =
		V44.bestCornerExit(position)

	if direction then
		V44.State.CornerActive = true
		V44.State.CornerDirection = direction
	end

	return direction
end

function V44.cornerRouteCost(
	oldPosition,
	newPosition,
	direction,
	cornerDirection
...
function V44.pushMotionHistory(data, position, now)
	...
	while #history > V44.C.RETURN_HISTORY_MAX do
		table.remove(history, 1)
	end
end

function V44.fitReturnArc(data)
	local history = data.MotionHistory
	...
	if radius < V44.C.RETURN_MIN_RADIUS
		or radius > V44.C.RETURN_MAX_RADIUS
	then
		return nil
	end
...
function V44.predictReturnArcPoint(data, t, origin)
	...
	local angle =
		rate
			* math.min(
				t,
				V44.C.RETURN_MAX_PREDICTION_TIME
			)
...
function V44.updateReturnModel(
	data,
	position,
	rawVelocity,
	now,
	...
	if spawnDistance >= V44.C.RETURN_MIN_TRAVEL then
		local future =
			V44.predictReturnArcPoint(
				data,
				V44.C.RETURN_PREVIEW_TIME,
				position
			)
...
local function buildThreatSnapshot(root, humanoid)
		...
		then
			...
			for stepIndex = 1, TOTAL_PREDICTION_STEPS do
				...
				if p0 and p1 then
					...
					if data.MotionClass == "RETURN"
						and t <= V44.C.RETURN_SIDE_LOOKAHEAD
	...
	V44.State.TrajectoryStraight = straightCount
	V44.State.TrajectoryCurve = curveCount
	V44.State.TrajectoryWave = waveCount
	V44.State.TrajectoryReturn = returnCount

	return threats
end

function V44.getShieldCharge()
...
function V44.getShieldCooldownRemaining()
	return math.max(
		0,
		V44.C.SHIELD_COOLDOWN
			- (os.clock() - V44.State.ShieldLastUse)
	)
end

function V44.findShieldPart()
	local core = workspace:FindFirstChild("Core__Game")
	if not core then
		return nil
	end
...
function V44.getActiveShieldModel()
	local now = os.clock()
	local shield = V44.findShieldPart()

	if shield then
		if V44.State.ShieldPart ~= shield then
			V44.State.ShieldPart = shield
			V44.State.ShieldCenter = shield.Position
			V44.State.ShieldUntil =
				now + V44.C.SHIELD_DURATION

			-- Also catches a manual shield use while the bot is running.
			if now - V44.State.ShieldLastUse > 1.0 then
				V44.State.ShieldLastUse = now
			end
		else
			V44.State.ShieldCenter = shield.Position
		end
		...
		local remaining =
			math.max(
				0,
				V44.State.ShieldUntil - now
			)

		if remaining > 0 then
			return {
				Center = V44.State.ShieldCenter,
				StartDelay = 0,
				Duration = remaining
			}
		end
	...
	if V44.State.ShieldPart ~= nil then
		V44.State.ShieldPart = nil
		V44.State.ShieldCenter = nil
		V44.State.ShieldUntil = 0
	end

	return nil
end

function V44.canUseShield()
	local charge =
		select(1, V44.getShieldCharge())

	if charge < V44.C.SHIELD_REQUIRED_CHARGE then
		V44.State.ShieldLastDecision = "NO CHARGE"
		return false
	end

	if V44.getShieldCooldownRemaining() > 0 then
		V44.State.ShieldLastDecision = "COOLDOWN"
		return false
	end

	if V44.getActiveShieldModel() then
		V44.State.ShieldLastDecision = "ACTIVE"
		return false
	end
	...
	if not event or not event:IsA("RemoteEvent") then
		V44.State.ShieldLastDecision = "NO REMOTE"
		return false
	end
...
function V44.useShield(root)
	if not root or not root.Parent then
		return false
	end

	if not V44.canUseShield() then
		return false
	end
	...
	V44.State.ShieldLastUse = now
	V44.State.ShieldCenter = root.Position
	V44.State.ShieldUntil =
		now
			+ V44.C.SHIELD_ACTIVATION_DELAY
			+ V44.C.SHIELD_DURATION

	V44.State.ShieldPlanHoldUntil =
		now + V44.C.SHIELD_PLAN_HOLD

	V44.State.ShieldUseCount += 1
	V44.State.ShieldLastDecision = "FIRED"

	event:FireServer(5, true)

	task.delay(
		V44.C.SHIELD_PRESS_TIME,
		function()
...
function V44.makeShieldThreatView(
	threats,
	center,
	startDelay,
	duration
...
local function constrainMoveAgainstBarriers(
	...
	then
		V44.State.WallConstraintState = "FREE"
	...
	V44.State.WallConstraintClearance =
	...
	V44.State.WallConstraintCount += 1

	if bestDirection then
		V44.State.WallConstraintState = "SLIDE"
		return bestDirection
	end

	V44.State.WallConstraintState = "STOP"
	return Vector3.zero
end
	...
	for _, threat in ipairs(activeThreats) do
		...
		if motionClass == "RETURN" then
			...
			local standoffClearance =
				clearance - V44.C.RETURN_STANDOFF
			...
			if clearance < V44.C.RETURN_CORRIDOR_ZONE then
				local tangent =
					flat(
						newProjectile
							- oldProjectile
				...
				if tangent.Magnitude > 0.01 then
					...
					if preferredSide ~= 0 then
						...
						if shrink > 0 then
							risk +=
								shrink * shrink
									* V44.C.RETURN_SIDE_SHRINK_WEIGHT
									* (0.35 + closeness)
						end
					else
						if playerMove.Magnitude > 0.01 then
							...
							risk +=
								along * along
									* V44.C.RETURN_CENTERLINE_TANGENT_WEIGHT
									* (0.35 + closeness)
						end
					end
		...
		elseif motionClass == "WAVE"
			and clearance
				< V44.C.RETURN_GRAZE_BLOCK_ZONE * 0.70
		then
...
local function getNextInputs(previousDirection, depth, emergency)
	...
	local finePhase =
		not emergency
		and stepEnd <= V44.C.FINE_STEER_END
	...
	V44.State.FineSteerActive = finePhase
...
local function evaluateGrazeStep(
	...
	if unstableGrazeBlock
		...
		or clearance >= V44.C.GRAZE_MAX_CLEARANCE
	then
		return 0, 0
	end
	...
	if clearance <= V44.C.GRAZE_HARD_FLOOR then
		local danger =
			1 - math.clamp(
				clearance
					/ V44.C.GRAZE_HARD_FLOOR,
				0,
				1
			)

		return
			V44.C.GRAZE_TOO_CLOSE_PENALTY
				* (0.35 + danger * danger),
			0
	end

	local target =
		V44.C.GRAZE_TARGET_CLEARANCE

	local width =
		math.max(
			0.01,
			V44.C.GRAZE_MAX_CLEARANCE
				- target
		)
...
local function nodeSort(a, b)
	...
	if GRAZE_MODE
		and not V44.State.CornerActive
		and a.Hits == 0
		and b.Hits == 0
...
local function pruneBeamWithDiversity(
	...
	if depth < V44.C.DIVERSITY_START_DEPTH
		or keep < 4
	then
		return beam, 0
	end
...
local function planPath(root, humanoid, threats, cornerDirection)
	...
	V44.State.DiversityInjected = 0
	V44.State.NearMissBest = 0
	V44.State.FineSteerActive = false
	V44.State.GrazeScoreBest = 0

	local grazePlanning =
		GRAZE_MODE
			and not V44.State.CornerActive

	V44.State.GrazePlanning = grazePlanning
		...
		for _, node in ipairs(beam) do
			...
			local emergency =
				...
				or node.MinReturnClearance
					< V44.C.RETURN_EMERGENCY_CLEARANCE
			local inputs =
				getNextInputs(
					node.Dir,
					depth,
	...
	table.sort(finalists, function(a, b)
		...
		then
			...
			if (
				aReturn < V44.C.RETURN_CORRIDOR_ZONE
				or bReturn < V44.C.RETURN_CORRIDOR_ZONE
			)
			...
			if CurrentMove.Magnitude > 0.01
				...
				and math.abs(costDelta)
					<= V44.C.ROBUST_CURRENT_MOVE_EPSILON
			then
				local aDot =
	...
	V44.State.NearMissBest =
		best.Node.NearMisses or 0

	V44.State.GrazeScoreBest =
		best.Node.GrazeScore or 0

	V44.State.ReturnMinClearance =
		best.Node.MinReturnClearance
			or math.huge

	V44.State.ReturnStandoffActive =
		V44.State.ReturnMinClearance
			< V44.C.RETURN_STANDOFF
...
Main.Size = UDim2.fromOffset(260, 405)
...
Info.Size = UDim2.new(1, -20, 0, 152)
...
Toggle.Position = UDim2.fromOffset(10, 210)
...
GrazeToggle.Position = UDim2.fromOffset(10, 255)
...
RadiusLabel.Position = UDim2.fromOffset(42, 296)
...
Minus.Position = UDim2.fromOffset(10, 296)
...
Plus.Position = UDim2.new(1, -38, 0, 296)
...
PathToggle.Position = UDim2.fromOffset(10, 337)
...
Footer.Position = UDim2.fromOffset(10, 376)
Footer.BackgroundTransparency = 1
Footer.Text = "v44 / Return Corridor + Robust Beam + Graze"
...
Toggle.Activated:Connect(function()
	...
	if ENABLED then
		...
		V44.State.CornerActive = false
		V44.State.CornerDirection = nil
		V44.State.ShieldPlanHoldUntil = 0
		V44.State.ShieldLastDecision = "IDLE"
		V44.State.WallConstraintState = "FREE"
		V44.State.ReturnMinClearance = math.huge
		V44.State.ReturnStandoffActive = false
		clearPathVisual()
		enableControls()
...
local function decisionStep()
	...
	if now < V44.State.ShieldPlanHoldUntil then
	...
	local ok, err = pcall(function()
		...
		if not activeShield
			and plan.Hits > 0
			and V44.canUseShield()
		then
			local urgentEnough =
				baseline.EarliestImpact ~= math.huge
				and baseline.EarliestImpact
					<= V44.C.SHIELD_TRIGGER_IMPACT

			local cornerEmergency =
				V44.State.CornerActive
			...
			if urgentEnough or cornerEmergency then
				...
				then
					if V44.useShield(root) then
						plan = shieldPlan
						shieldUsed = true
						V44.State.ShieldLastDecision =
							"BREAKOUT"
					end
				else
					V44.State.ShieldLastDecision =
		...
		elseif plan.Hits > 0 then
			LastMode = "RESCUE"
		elseif V44.State.CornerActive then
		...
		Info.Text =
			...
			.. (
				V44.getActiveShieldModel()
					and "ACTIVE"
					or (
						V44.getShieldCooldownRemaining() > 0
							and (
								"CD "
								.. string.format(
									"%.0f",
									V44.getShieldCooldownRemaining()
								)
							)
							or V44.State.ShieldLastDecision
					)
			)
			.. " / Del "
			.. tostring(V44.State.ShieldLastRemoved)
			.. " / Sim "
			.. string.format(
				"%.1fms",
				V44.State.ShieldSimMs
			...
			.. "\nSearch D"
			.. tostring(V44.State.DiversityInjected)
			.. " / NM "
			.. tostring(V44.State.NearMissBest)
			.. " / Fine "
			.. (
				V44.State.FineSteerActive
					and "ON"
					or "-"
			)
			.. "\nGraze "
			...
			.. " / Fix "
			.. tostring(
				V44.State.WallConstraintCount
			)
			.. "\nTraj S"
			.. tostring(
				V44.State.TrajectoryStraight
			)
			.. " C"
			.. tostring(
				V44.State.TrajectoryCurve
			)
			.. " W"
			.. tostring(
				V44.State.TrajectoryWave
			)
			.. " R"
			.. tostring(
				V44.State.TrajectoryReturn
			)
			.. "\nRetPath "
			.. (
				V44.State.ReturnMinClearance
