...
C.IMMEDIATE_MPC_DIRECTIONS = 16
C.IMMEDIATE_MPC_BEAM_WIDTH = 20
C.IMMEDIATE_MPC_DENSE_BEAM_WIDTH = 16
C.IMMEDIATE_MPC_MAX_THREATS = 80
...
local function applyFocusState(wanted, force)
	...
	local now = os.clock()
	if not force
		and now - LastFocusToggleAt < C.FOCUS_MIN_HOLD_TIME
	then
		return false
	end
...
local function getPredictedWalkSpeed(humanoid, focus)
	...
	local normal =
		...
		or (
			...
			and currentWalkSpeed
				/ math.max(C.FOCUS_FALLBACK_SPEED_RATIO, 0.05)
			or currentWalkSpeed
		)
	...
	local focused =
		FocusSpeedEstimate
		or normal * C.FOCUS_FALLBACK_SPEED_RATIO
...
local function getFocusOptions(node, allowFocus, emergency)
	...
	if age < C.FOCUS_MIN_HOLD_TIME and not emergency then
		return {current}
	end
...
local function focusTransitionCost(oldFocus, newFocus, dt, emergency)
	...
	if newFocus then
		cost += C.FOCUS_ACTIVE_COST_PER_SECOND * dt
	end

	if oldFocus ~= newFocus then
		cost +=
			emergency
			and C.FOCUS_TOGGLE_COST * 0.35
			or C.FOCUS_TOGGLE_COST
	end
...
local function getNetworkLead()
	...
	return math.clamp(lead, C.NETWORK_LEAD_MIN, C.NETWORK_LEAD_MAX)
end
...
local function planImmediateMPC(root, humanoid, threats, fanInfo)
	...
	local beamWidth =
		dense
		and C.IMMEDIATE_MPC_DENSE_BEAM_WIDTH
		or C.IMMEDIATE_MPC_BEAM_WIDTH
...
local function planPath(root, humanoid, threats, denseFront)
	...
	for depth = 1, C.PLAN_STEPS do
		...
		if os.clock() - sliceStarted >= C.PLANNER_SLICE_BUDGET then
			RunService.Heartbeat:Wait()
			sliceStarted = os.clock()
		end
	end
...
Footer.Text = "v16 / Spatial MPC / Native Focus"
