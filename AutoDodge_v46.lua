...
local V46 = {
	C = {
		...
		DODGE_COMMIT_TIME = 0.30,
		DODGE_COMMIT_TIME_GRAZE = 0.16,
		DODGE_COMMIT_MAX_HOLD = 0.46,
		DODGE_COMMIT_REACH = 0.38,
		DODGE_COMMIT_MIN_DISTANCE = 0.60,
		DODGE_COMMIT_REPLAN_CLEARANCE = 0.20
	},
	State = {
		CornerActive = false,
		CornerDirection = nil,
		...
		DodgeCommitTarget = nil,
		DodgeCommitUntil = 0,
		DodgeCommitDistance = 0,
		DodgeCommitCount = 0,
		DodgeCommitState = "FREE"
	}
}
...
function V46.directionOpenDistance(position, direction)
	local nearest = V46.C.CORNER_PROBE_DISTANCE
...
function V46.bestCornerExit(position)
	local bestDirection = nil
	local bestScore = -math.huge

	for i = 0, V46.C.CORNER_PROBE_DIRECTIONS - 1 do
		local angle =
			i * math.pi * 2
				/ V46.C.CORNER_PROBE_DIRECTIONS
...
function V46.updateCornerObjective(position)
	local _, minimumOpen, closeRays =
		barrierOpenness(position)

	-- Once we have actually left the corner, release the objective.
	if minimumOpen >= V46.C.CORNER_RELEASE_OPEN
		and closeRays < V46.C.CORNER_CLOSE_RAYS
	then
		V46.State.CornerActive = false
		V46.State.CornerDirection = nil
		return nil
	end

	local trapped =
		minimumOpen < V46.C.CORNER_MIN_OPEN
		and closeRays >= V46.C.CORNER_CLOSE_RAYS

	-- Critical anti-wiggle behavior:
	-- keep the SAME exit direction as long as that channel still exists.
	if V46.State.CornerActive
		and V46.State.CornerDirection
	then
		local clearance =
			V46.directionOpenDistance(
				position,
				V46.State.CornerDirection
			)

		if clearance
			>= V46.C.CORNER_EXIT_MIN_CLEARANCE
		then
			return V46.State.CornerDirection
		end
	...
	local direction =
		V46.bestCornerExit(position)

	if direction then
		V46.State.CornerActive = true
		V46.State.CornerDirection = direction
	end

	return direction
end

function V46.cornerRouteCost(
	oldPosition,
	newPosition,
	direction,
	cornerDirection
...
function V46.pushMotionHistory(data, position, now)
	...
	while #history > V46.C.RETURN_HISTORY_MAX do
		table.remove(history, 1)
	end
end

function V46.fitReturnArc(data)
	local history = data.MotionHistory
	...
	if radius < V46.C.RETURN_MIN_RADIUS
		or radius > V46.C.RETURN_MAX_RADIUS
	then
		return nil
	end
...
function V46.predictReturnArcPoint(data, t, origin)
	...
	local angle =
		rate
			* math.min(
				t,
				V46.C.RETURN_MAX_PREDICTION_TIME
			)
...
function V46.updateReturnModel(
	data,
	position,
	rawVelocity,
	now,
	...
	if spawnDistance >= V46.C.RETURN_MIN_TRAVEL then
		local future =
			V46.predictReturnArcPoint(
				data,
				V46.C.RETURN_PREVIEW_TIME,
				position
			)
...
local function buildThreatSnapshot(root, humanoid)
		...
		then
			...
			for stepIndex = 1, TOTAL_PREDICTION_STEPS do
				...
				if p0 and p1 then
					...
					if data.MotionClass == "RETURN"
						and t <= V46.C.RETURN_SIDE_LOOKAHEAD
	...
	V46.State.TrajectoryStraight = straightCount
	V46.State.TrajectoryCurve = curveCount
	V46.State.TrajectoryWave = waveCount
	V46.State.TrajectoryReturn = returnCount

	return threats
end

function V46.getShieldCharge()
...
function V46.getShieldCooldownRemaining()
	return math.max(
		0,
		V46.C.SHIELD_COOLDOWN
			- (os.clock() - V46.State.ShieldLastUse)
	)
end

function V46.findShieldPart()
	local core = workspace:FindFirstChild("Core__Game")
	if not core then
		return nil
	end
...
function V46.getActiveShieldModel()
	local now = os.clock()
	local shield = V46.findShieldPart()

	if shield then
		if V46.State.ShieldPart ~= shield then
			V46.State.ShieldPart = shield
			V46.State.ShieldCenter = shield.Position
			V46.State.ShieldUntil =
				now + V46.C.SHIELD_DURATION

			-- Also catches a manual shield use while the bot is running.
			if now - V46.State.ShieldLastUse > 1.0 then
				V46.State.ShieldLastUse = now
			end
		else
			V46.State.ShieldCenter = shield.Position
		end
		...
		local remaining =
			math.max(
				0,
				V46.State.ShieldUntil - now
			)

		if remaining > 0 then
			return {
				Center = V46.State.ShieldCenter,
				StartDelay = 0,
				Duration = remaining
			}
		end
	...
	if V46.State.ShieldPart ~= nil then
		V46.State.ShieldPart = nil
		V46.State.ShieldCenter = nil
		V46.State.ShieldUntil = 0
	end

	return nil
end

function V46.canUseShield()
	local charge =
		select(1, V46.getShieldCharge())

	if charge < V46.C.SHIELD_REQUIRED_CHARGE then
		V46.State.ShieldLastDecision = "NO CHARGE"
		return false
	end

	if V46.getShieldCooldownRemaining() > 0 then
		V46.State.ShieldLastDecision = "COOLDOWN"
		return false
	end

	if V46.getActiveShieldModel() then
		V46.State.ShieldLastDecision = "ACTIVE"
		return false
	end
	...
	if not event or not event:IsA("RemoteEvent") then
		V46.State.ShieldLastDecision = "NO REMOTE"
		return false
	end
...
function V46.useShield(root)
	if not root or not root.Parent then
		return false
	end

	if not V46.canUseShield() then
		return false
	end
	...
	V46.State.ShieldLastUse = now
	V46.State.ShieldCenter = root.Position
	V46.State.ShieldUntil =
		now
			+ V46.C.SHIELD_ACTIVATION_DELAY
			+ V46.C.SHIELD_DURATION

	V46.State.ShieldPlanHoldUntil =
		now + V46.C.SHIELD_PLAN_HOLD

	V46.State.ShieldUseCount += 1
	V46.State.ShieldLastDecision = "FIRED"

	event:FireServer(5, true)

	task.delay(
		V46.C.SHIELD_PRESS_TIME,
		function()
...
function V46.makeShieldThreatView(
	threats,
	center,
	startDelay,
	duration
...
local function constrainMoveAgainstBarriers(
	...
	then
		V46.State.WallConstraintState = "FREE"
	...
	V46.State.WallConstraintClearance =
	...
	V46.State.WallConstraintCount += 1

	if bestDirection then
		V46.State.WallConstraintState = "SLIDE"
		return bestDirection
	end

	V46.State.WallConstraintState = "STOP"
	return Vector3.zero
end
	...
	for _, threat in ipairs(activeThreats) do
		...
		if motionClass == "RETURN" then
			...
			local standoffClearance =
				clearance - V46.C.RETURN_STANDOFF
			...
			if clearance < V46.C.RETURN_CORRIDOR_ZONE then
				local tangent =
					flat(
						newProjectile
							- oldProjectile
				...
				if tangent.Magnitude > 0.01 then
					...
					if preferredSide ~= 0 then
						...
						if shrink > 0 then
							risk +=
								shrink * shrink
									* V46.C.RETURN_SIDE_SHRINK_WEIGHT
									* (0.35 + closeness)
						end
					else
						if playerMove.Magnitude > 0.01 then
							...
							risk +=
								along * along
									* V46.C.RETURN_CENTERLINE_TANGENT_WEIGHT
									* (0.35 + closeness)
						end
					end
		...
		elseif motionClass == "WAVE"
			and clearance
				< V46.C.RETURN_GRAZE_BLOCK_ZONE * 0.70
		then
...
local function getNextInputs(previousDirection, depth, emergency)
	...
	local finePhase =
		not emergency
		and stepEnd <= V46.C.FINE_STEER_END
	...
	V46.State.FineSteerActive = finePhase
...
local function evaluateGrazeStep(
	...
	if unstableGrazeBlock
		...
		or clearance >= V46.C.GRAZE_MAX_CLEARANCE
	then
		return 0, 0
	end
	...
	if clearance <= V46.C.GRAZE_HARD_FLOOR then
		local danger =
			1 - math.clamp(
				clearance
					/ V46.C.GRAZE_HARD_FLOOR,
				0,
				1
			)

		return
			V46.C.GRAZE_TOO_CLOSE_PENALTY
				* (0.35 + danger * danger),
			0
	end

	local target =
		V46.C.GRAZE_TARGET_CLEARANCE

	local width =
		math.max(
			0.01,
			V46.C.GRAZE_MAX_CLEARANCE
				- target
		)
...
local function nodeSort(a, b)
	...
	if GRAZE_MODE
		and not V46.State.CornerActive
		and a.Hits == 0
		and b.Hits == 0
...
local function pruneBeamWithDiversity(
	...
	if depth < V46.C.DIVERSITY_START_DEPTH
		or keep < 4
	then
		return beam, 0
	end
...
local function planPath(root, humanoid, threats, cornerDirection)
	...
	V46.State.DiversityInjected = 0
	V46.State.NearMissBest = 0
	V46.State.FineSteerActive = false
	V46.State.GrazeScoreBest = 0

	local grazePlanning =
		GRAZE_MODE
			and not V46.State.CornerActive

	V46.State.GrazePlanning = grazePlanning
		...
		for _, node in ipairs(beam) do
			...
			local emergency =
				...
				or node.MinReturnClearance
					< V46.C.RETURN_EMERGENCY_CLEARANCE
			local inputs =
				getNextInputs(
					node.Dir,
					depth,
	...
	table.sort(finalists, function(a, b)
		...
		then
			...
			if (
				aReturn < V46.C.RETURN_CORRIDOR_ZONE
				or bReturn < V46.C.RETURN_CORRIDOR_ZONE
			)
			...
			if CurrentMove.Magnitude > 0.01
				...
				and math.abs(costDelta)
					<= V46.C.ROBUST_CURRENT_MOVE_EPSILON
			then
				local aDot =
	...
	V46.State.NearMissBest =
		best.Node.NearMisses or 0

	V46.State.GrazeScoreBest =
		best.Node.GrazeScore or 0

	V46.State.ReturnMinClearance =
		best.Node.MinReturnClearance
			or math.huge

	V46.State.ReturnStandoffActive =
		V46.State.ReturnMinClearance
			< V46.C.RETURN_STANDOFF

	local commitTime =
		grazePlanning
			and V46.C.DODGE_COMMIT_TIME_GRAZE
			or V46.C.DODGE_COMMIT_TIME
...
function V46.clearDodgeCommit()
	V46.State.DodgeCommitTarget = nil
	V46.State.DodgeCommitUntil = 0
	V46.State.DodgeCommitDistance = 0
	V46.State.DodgeCommitState = "FREE"
end

function V46.setDodgeCommit(root, plan)
	local target =
		plan and plan.CommitTarget

	if not root or not target then
		V46.clearDodgeCommit()
		return false
	end

	local delta =
		flat(target - root.Position)

	local distance = delta.Magnitude

	if distance
		< V46.C.DODGE_COMMIT_MIN_DISTANCE
	then
		V46.clearDodgeCommit()
		return false
	end

	V46.State.DodgeCommitTarget = target
	V46.State.DodgeCommitUntil =
		os.clock()
			+ V46.C.DODGE_COMMIT_MAX_HOLD
	V46.State.DodgeCommitDistance = distance
	V46.State.DodgeCommitCount += 1
	V46.State.DodgeCommitState = "ARMED"

	return true
end

function V46.continueDodgeCommit(root)
	local target =
		V46.State.DodgeCommitTarget
	...
	if distance <= V46.C.DODGE_COMMIT_REACH then
		V46.clearDodgeCommit()
		return false
	end

	-- Do not blindly finish an old waypoint if the straight continuation
	-- became physically invalid. The wall output constraint remains the
	-- final guard, but dropping the stale commitment here prevents grinding.
	if pathHitsBarrier(
		root.Position,
		target
	) then
	...
	CurrentMove = delta.Unit
	V46.State.DodgeCommitState = "FOLLOW"
	LastMode = "MPC COMMIT"
...
Footer.Text = "v46 / MPC Commit + Crisis Shield + Return Corridor"
...
Toggle.Activated:Connect(function()
	...
	if ENABLED then
		...
		V46.State.CornerActive = false
		V46.State.CornerDirection = nil
		V46.State.ShieldPlanHoldUntil = 0
		V46.State.ShieldLastDecision = "IDLE"
		V46.State.ShieldCrisisReason = "-"
		V46.State.WallConstraintState = "FREE"
		V46.State.ReturnMinClearance = math.huge
		V46.State.ReturnStandoffActive = false
		V46.clearDodgeCommit()
		clearPathVisual()
		enableControls()
...
local function decisionStep()
	...
	if now < V46.State.ShieldPlanHoldUntil then
	...
	local ok, err = pcall(function()
		...
		then
			...
			if V46.continueDodgeCommit(root) then
				applyCurrentMove(
					humanoid,
					root
				)
		...
		then
			...
			local imminentPlanHit =
				unavoidable
				and firstImpact
					<= V46.C.SHIELD_PANIC_IMPACT

			local urgentPlanHit =
				unavoidable
				and firstImpact
					<= V46.C.SHIELD_TRIGGER_IMPACT

			local fragilePlan =
				plan.Hits == 0
				and plan.MinClearance
					<= V46.C.SHIELD_FRAGILE_CLEARANCE
				and plan.EscapeOptions
					<= V46.C.SHIELD_FRAGILE_ESCAPE_OPTIONS

			local trappedPlan =
				V46.State.CornerActive
				and plan.EscapeOptions
					<= V46.C.SHIELD_FRAGILE_ESCAPE_OPTIONS
			...
			if crisis then
				if imminentPlanHit then
					V46.State.ShieldCrisisReason = "PANIC"
				elseif urgentPlanHit then
					V46.State.ShieldCrisisReason = "URGENT"
				elseif unavoidable then
					V46.State.ShieldCrisisReason = "UNAVOIDABLE"
				elseif trappedPlan then
					V46.State.ShieldCrisisReason = "TRAPPED"
				elseif fragilePlan then
					V46.State.ShieldCrisisReason = "FRAGILE"
				else
					V46.State.ShieldCrisisReason = "BASELINE"
				end
				...
				local materiallySafer =
					shieldMakesSafe
						and (
							unavoidable
							or clearanceGain
								>= V46.C.SHIELD_CLEARANCE_GAIN
							or escapeGain
								>= V46.C.SHIELD_ESCAPE_GAIN
						)
		...
		elseif activeShield then
			V46.State.ShieldCrisisReason = "ACTIVE"
			V46.State.ShieldLastDecision =
				"ACTIVE"
		else
		...
		if shieldUsed then
			if V46.State.ShieldLastDecision == "PANIC FIRE" then
				LastMode = "SHIELD PANIC"
			elseif V46.State.ShieldLastDecision == "RESCUE" then
				LastMode = "SHIELD RESCUE"
			else
				LastMode = "SHIELD BREAKOUT"
			end
		elseif GRAZE_MODE
			and V46.State.GrazePlanning
			and plan.Hits == 0
		...
		elseif plan.Hits > 0 then
			LastMode = "RESCUE"
		elseif V46.State.CornerActive then
		...
		Info.Text =
			...
			.. (
				V46.getActiveShieldModel()
					and "ACTIVE"
					or (
						V46.getShieldCooldownRemaining() > 0
							and (
								"CD "
								.. string.format(
									"%.0f",
									V46.getShieldCooldownRemaining()
								)
							)
							or V46.State.ShieldLastDecision
					)
			)
			.. " / "
			.. V46.State.ShieldCrisisReason
			.. " / Del "
			.. tostring(V46.State.ShieldLastRemoved)
			.. " / Sim "
			.. string.format(
				"%.1fms",
				V46.State.ShieldSimMs
			...
			.. "\nSearch D"
			.. tostring(V46.State.DiversityInjected)
			.. " / NM "
			.. tostring(V46.State.NearMissBest)
			.. " / Fine "
			.. (
				V46.State.FineSteerActive
					and "ON"
					or "-"
			)
			.. "\nGraze "
			...
			.. " / Fix "
			.. tostring(
				V46.State.WallConstraintCount
			)
			.. "\nTraj S"
			.. tostring(
				V46.State.TrajectoryStraight
			)
			.. " C"
			.. tostring(
				V46.State.TrajectoryCurve
			)
			.. " W"
			.. tostring(
				V46.State.TrajectoryWave
			)
			.. " R"
			.. tostring(
				V46.State.TrajectoryReturn
			)
			.. "\nRetPath "
			.. (
				V46.State.ReturnMinClearance
			...
			.. "\nCommit "
			.. V46.State.DodgeCommitState
			.. " / "
			.. string.format(
				"%.2f",
				V46.State.DodgeCommitDistance
			)
			.. " / #"
			.. tostring(
				V46.State.DodgeCommitCount
			)
			.. (
				LastError ~= ""
