...
warn("[AutoDodge v26] boot / CLASSIC Beam + FAN SKIRT")
	...
	for depth = 1, C.PLAN_STEPS do
		...
		for _, node in ipairs(beam) do
			...
			for _, inputDirection in ipairs(inputs) do
				...
				-- v5 continuity only. No reverse lock, no hysteresis.
				if depth == 1
...
Footer.Text = "v26 / CLASSIC Beam / Slow FAN Skirt"
...
local function decisionStep()
	...
	local ok, err = pcall(function()
		...
		-- Keep the newer tracker / spatial broad phase, but feed it directly
		-- into the old v5 planner with no background yield pipeline.
		local threats =
			buildThreatSnapshot(
				root,
				humanoid
...
-- v25 CLASSIC: exactly one movement brain.
-- The full 1.5 s multi-turn Beam MPC is called synchronously from RenderStep.
-- No Immediate MPC, no Emergency override, no FAN controller.
