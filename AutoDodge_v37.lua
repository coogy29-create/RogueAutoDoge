...
function V37.directionOpenDistance(position, direction)
	local nearest = V37.C.CORNER_PROBE_DISTANCE
...
function V37.bestCornerExit(position)
	local bestDirection = nil
	local bestScore = -math.huge

	for i = 0, V37.C.CORNER_PROBE_DIRECTIONS - 1 do
		local angle =
			i * math.pi * 2
				/ V37.C.CORNER_PROBE_DIRECTIONS
...
function V37.updateCornerObjective(position)
	local _, minimumOpen, closeRays =
		barrierOpenness(position)

	-- Once we have actually left the corner, release the objective.
	if minimumOpen >= V37.C.CORNER_RELEASE_OPEN
		and closeRays < V37.C.CORNER_CLOSE_RAYS
	then
		V37.State.CornerActive = false
		V37.State.CornerDirection = nil
		return nil
	end

	local trapped =
		minimumOpen < V37.C.CORNER_MIN_OPEN
		and closeRays >= V37.C.CORNER_CLOSE_RAYS

	-- Critical anti-wiggle behavior:
	-- keep the SAME exit direction as long as that channel still exists.
	if V37.State.CornerActive
		and V37.State.CornerDirection
	then
		local clearance =
			V37.directionOpenDistance(
				position,
				V37.State.CornerDirection
			)

		if clearance
			>= V37.C.CORNER_EXIT_MIN_CLEARANCE
		then
			return V37.State.CornerDirection
		end
	...
	local direction =
		V37.bestCornerExit(position)

	if direction then
		V37.State.CornerActive = true
		V37.State.CornerDirection = direction
	end

	return direction
end

function V37.cornerRouteCost(
	oldPosition,
	newPosition,
	direction,
	cornerDirection
...
function V37.getShieldCooldownRemaining()
	return math.max(
		0,
		V37.C.SHIELD_COOLDOWN
			- (os.clock() - V37.State.ShieldLastUse)
	)
end

function V37.findShieldPart()
	local core = workspace:FindFirstChild("Core__Game")
	if not core then
		return nil
	end
...
function V37.getActiveShieldModel()
	local now = os.clock()
	local shield = V37.findShieldPart()

	if shield then
		if V37.State.ShieldPart ~= shield then
			V37.State.ShieldPart = shield
			V37.State.ShieldCenter = shield.Position
			V37.State.ShieldUntil =
				now + V37.C.SHIELD_DURATION

			-- Also catches a manual shield use while the bot is running.
			if now - V37.State.ShieldLastUse > 1.0 then
				V37.State.ShieldLastUse = now
			end
		else
			V37.State.ShieldCenter = shield.Position
		end
		...
		local remaining =
			math.max(
				0,
				V37.State.ShieldUntil - now
			)

		if remaining > 0 then
			return {
				Center = V37.State.ShieldCenter,
				StartDelay = 0,
				Duration = remaining
			}
		end
	...
	if V37.State.ShieldPart ~= nil then
		V37.State.ShieldPart = nil
		V37.State.ShieldCenter = nil
		V37.State.ShieldUntil = 0
	end

	return nil
end

function V37.canUseShield()
	local charge =
		select(1, V37.getShieldCharge())

	if charge < V37.C.SHIELD_REQUIRED_CHARGE then
		V37.State.ShieldLastDecision = "NO CHARGE"
		return false
	end

	if V37.getShieldCooldownRemaining() > 0 then
		V37.State.ShieldLastDecision = "COOLDOWN"
		return false
	end

	if V37.getActiveShieldModel() then
		V37.State.ShieldLastDecision = "ACTIVE"
		return false
	end
	...
	if not event or not event:IsA("RemoteEvent") then
		V37.State.ShieldLastDecision = "NO REMOTE"
		return false
	end
...
function V37.useShield(root)
	if not root or not root.Parent then
		return false
	end

	if not V37.canUseShield() then
		return false
	end
	...
	V37.State.ShieldLastUse = now
	V37.State.ShieldCenter = root.Position
	V37.State.ShieldUntil =
		now
			+ V37.C.SHIELD_ACTIVATION_DELAY
			+ V37.C.SHIELD_DURATION

	V37.State.ShieldPlanHoldUntil =
		now + V37.C.SHIELD_PLAN_HOLD

	V37.State.ShieldUseCount += 1
	V37.State.ShieldLastDecision = "FIRED"

	event:FireServer(5, true)

	task.delay(
		V37.C.SHIELD_PRESS_TIME,
		function()
...
function V37.makeShieldThreatView(
	threats,
	center,
	startDelay,
	duration
...
Footer.Text = "v37 / Clean V4 + Corner Exit + Smart Shield"
...
Toggle.Activated:Connect(function()
	...
	if ENABLED then
		...
		V37.State.CornerActive = false
		V37.State.CornerDirection = nil
		V37.State.ShieldPlanHoldUntil = 0
		V37.State.ShieldLastDecision = "IDLE"
		clearPathVisual()
		enableControls()
...
local function decisionStep()
	...
	if now < V37.State.ShieldPlanHoldUntil then
	...
	local ok, err = pcall(function()
		...
		if not activeShield
			and plan.Hits > 0
			and V37.canUseShield()
		then
			local urgentEnough =
				baseline.EarliestImpact ~= math.huge
				and baseline.EarliestImpact
					<= V37.C.SHIELD_TRIGGER_IMPACT

			local cornerEmergency =
				V37.State.CornerActive
			...
			if urgentEnough or cornerEmergency then
				...
				then
					if V37.useShield(root) then
						plan = shieldPlan
						shieldUsed = true
						V37.State.ShieldLastDecision =
							"BREAKOUT"
					end
				else
					V37.State.ShieldLastDecision =
		...
		elseif plan.Hits > 0 then
			LastMode = "RESCUE"
		elseif V37.State.CornerActive then
		...
		Info.Text =
			...
			.. (
				V37.getActiveShieldModel()
					and "ACTIVE"
					or (
						V37.getShieldCooldownRemaining() > 0
							and (
								"CD "
								.. string.format(
									"%.0f",
									V37.getShieldCooldownRemaining()
								)
							)
							or V37.State.ShieldLastDecision
					)
			)
			.. " / Del "
			.. tostring(V37.State.ShieldLastRemoved)
			.. " / Sim "
			.. string.format(
				"%.1fms",
				V37.State.ShieldSimMs
